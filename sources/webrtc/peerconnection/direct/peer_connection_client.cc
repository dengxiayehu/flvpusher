...
// Delay between server connction retries, in milliseconds
const int kReconnectDelay = 2000;

const int kReconnectMessage = 0;
...
PeerConnectionClient::PeerConnectionClient() : callback_(NULL), resolver_(NULL), state_(NOT_CONNECTED), my_id_(-1) {
}
...
void PeerConnectionClient::Connect(const std::string& server, int port, const std::string& client_name) {
    ...
    if (state_ != NOT_CONNECTED) {
        LOG(WARNING) << "The client must not be connected before you call Connect()";
...
bool PeerConnectionClient::SignOut() {
    ...
    if (control_socket_->GetState() == rtc::Socket::CS_CLOSED) {
        ...
        if (my_id_ != -1) {
            char buffer[1024];
            sprintfn(buffer, sizeof(buffer), "GET /sign_out?peer_id=%i HTTP/1.0\r\n\r\n", my_id_);
            onconnect_data_ = buffer;
            return ConnectControlSocket();
        } else {
...
void PeerConnectionClient::OnClose(rtc::AsyncSocket* socket, int err) {
    ...
    if (err != ECONNREFUSED) {
        ...
        if (socket == control_socket_.get()) {
            LOG(WARNING) << "Connection resused, retrying in 2 seconds";
            rtc::Thread::Current()->PostDelayed(kReconnectDelay, this, kReconnectMessage);
        } else {
            Close();
            callback_->OnDisconnected();
        }
...
void PeerConnectionClient::OnHangingGetRead(rtc::AsyncSocket* socket) {
    ...
    // Wait for the incoming message again.
