...
void FakeMainWnd::MessageBox(const char* caption, const char* text, bool is_error) {
    LOG(LERROR) << "*** " << caption << " ***\n" << text;
}
...
FakeMainWnd::VideoRenderer::VideoRenderer(FakeMainWnd* main_wnd, webrtc::VideoTrackInterface* track_to_render)
...
void FakeMainWnd::VideoRenderer::RenderFrame(const cricket::VideoFrame* video_frame) {
    {
        rtc::CritScope cs(&main_wnd_->crit_);
        ...
        memcpy(image_.get(), frame->GetYPlane(), frame->GetYPitch() * frame->GetHeight());
        memcpy(image_.get() + frame->GetYPitch() * frame->GetHeight(), frame->GetUPlane(),
               frame->GetUPitch() * frame->GetHeight() / 2);
