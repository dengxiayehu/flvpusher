...
bool Conductor::InitializePeerConnection() {
    ...
    fake_audio_capture_module_ = FakeAudioCaptureModule::Create(rtc::Thread::Current());

    if (fake_audio_capture_module_ == NULL) {
        main_wnd_->MessageBox("Error", "Failed to create FakeAudioCaptureModule", true);
        goto bail;
    }
...
bool Conductor::ReinitializePeerConnectionForLoopback() {
    loopback_ = true;
    rtc::scoped_refptr<webrtc::StreamCollectionInterface> streams(peer_connection_->local_streams());
...
void Conductor::ConnectToPeer(int peer_id) {
    ...
    if (InitializePeerConnection()) {
        peer_id_ = peer_id;
        webrtc::FakeConstraints constraints;
        constraints.AddOptional(webrtc::MediaConstraintsInterface::kUseRtpMux, "false");
...
void Conductor::AddStreams() {
    ...
    cricket::FakeVideoCapturer* fake_capturer = new webrtc::FakePeriodicVideoCapturer();
    rtc::scoped_refptr<webrtc::VideoSourceInterface> source =
            peer_connection_factory_->CreateVideoSource(fake_capturer, &source_constraints);
    rtc::scoped_refptr<webrtc::VideoTrackInterface> video_track(
            peer_connection_factory_->CreateVideoTrack(kVideoLabel, source));
