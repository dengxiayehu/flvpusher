...
int App::init() {
    if (m_sig_hdl->install(sighandler, SIGINT, SIGALRM, Signaler::SIGLIST_END) != SUCCESS) {
        LOGE("Install SIGINT's handler failed");
        return -1;
    }
    if (m_sig_hdl->install(SIG_IGN, SIGPIPE, Signaler::SIGLIST_END) != SUCCESS) {
...
int App::load_cfg(std::string cfg_file) {
    if (!is_path_absolute(STR(cfg_file))) {
        cfg_file = sprintf_("%s%c%s", STR(dirname_(abs_program)), DIRSEP, STR(cfg_file));
    }
    if (!is_file(cfg_file)) {
...
int App::parse_arg(int argc, char* argv[]) {
    struct option longopts[] = {
            {"input", required_argument, NULL, 'i'},    {"live", required_argument, NULL, 'L'},
            {"help", required_argument, NULL, 'h'},     {"dvfile", required_argument, NULL, 'v'},
            {"dafile", required_argument, NULL, 'a'},   {"hls_playlist", required_argument, NULL, 'p'},
            {"hls_time", required_argument, NULL, 't'}, {"hls_segment", required_argument, NULL, 'S'},
            {"hls_m3u8", required_argument, NULL, 'm'}, {"loop", no_argument, NULL, 'T'},
            {"tspath", required_argument, NULL, 's'},   {"flvpath", required_argument, NULL, 'f'},
            {"logpath", required_argument, NULL, 'l'},  {"no_logfile", no_argument, NULL, 'N'},
            {"webserver", no_argument, NULL, 'w'},      {0, 0, 0, 0}};
    int ch;
...
int App::check_arg() const {
    if (!m_req_segment.empty() || !m_req_m3u8.empty()) {
        return 0;
    }
    ...
    if ((m_liveurl.empty() && m_hls_playlist.empty()) || (!m_liveurl.empty() && !m_hls_playlist.empty())) {
...
void App::usage() const {
    fprintf(stderr,
            "flvpusher (V: %s)\n\n"
            "Usage: flvpusher <-i source|-w> <-L liveurl [--loop] [-a dump_audio] [-v dump_video] [-s tspath] [-f "
            "flvpath]|--hls_playlist filename [--hls_time time]> [-h] [--no_logfile]\n"
            "Description: \n"
            "-i, --input\n"
...
int App::main(int argc, char* argv[]) {
    ...
    if (parse_arg(argc, argv) < 0 || check_arg() < 0) {
        usage();
        return 1;
    }
    ...
    if (m_webserver) {
        ...
        int listen_port = DEFAULT_LISTEN_PORT, server_threads = DEFAULT_SERVER_THREADS;
        if (m_conf) {
    ...
    std::vector<std::string> input(xutil::split(m_input_str, INPUT_SEPARATOR));
    ...
    if (!m_hls_playlist.empty()) {
        ...
        m_hls = new HLSSegmenter(m_hls_playlist,  // Path of *.m3u8
    ...
    while (!interrupt_cb()) {
        for (auto it = input.begin(); it != input.end(); it++) {
            ...
            if (!m_pusher) {
                LOGE("Media file \"%s\" not supported (ignored)", STR(*it));
                continue;
            }
