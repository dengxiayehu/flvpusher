...
int FLVParser::read_header(FLVHeader& hdr, uint8_t* buf, uint32_t buf_size) {
    if (!buf) {
        if (!m_file.read_buffer(reinterpret_cast<uint8_t*>(&hdr), sizeof(FLVHeader))) {
    ...
    if (hdr.dataoffset != 9) {
        LOGE("FLV version 1 should have 9 bytes flv_header, not %d bytes", hdr.dataoffset);
        return -1;
    }
...
int FLVParser::read_tag(FLVTag*& tag, uint8_t* buf, uint32_t buf_size) {
    if (!buf) {
        if (!m_file.read_buffer(reinterpret_cast<uint8_t*>(&tag->hdr), sizeof(FLVTagHeader))) {
...
#ifdef XDEBUG
    LOGD("Tag type: %s", tag->hdr.typ == TAG_AUDIO ? "Audio" : (tag->hdr.typ == TAG_VIDEO ? "Video" : "Script"));
    ...
    if (tag->hdr.typ == TAG_VIDEO) {
        LOGD("Tag composition-time: %u", VALUI24(tag->dat.video.pkt.composition_time));
    }
#endif
    ...
    if (prev_tag_size != VALUI24(tag->hdr.datasize) + sizeof(FLVTagHeader)) {
        LOGE("Read \"Prev-Tag-Size\" failed, %u != %u + %u", prev_tag_size, VALUI24(tag->hdr.datasize),
             sizeof(FLVTagHeader));
        return -1;
    }
...
int FLVParser::parse_avc(const byte*& p, uint32_t len, FLVVideoTagData& vdat) {
    ...
    if (vdat.pkt.pkt_typ != NALU && composition_time != 0) {
        LOGE("AVCPktType %u should with 0 composition_time, not %u", vdat.pkt.pkt_typ, composition_time);
        return -1;
    }
...
int FLVParser::handle_video(FLVVideoTagData& vdat, const byte strm[], uint32_t len) {
    const byte* p = strm;
...
int FLVParser::handle_audio(FLVAudioTagData& adat, const byte strm[], uint32_t len) {
    const byte* p = strm;
    ...
    *(byte*)&adat = *p++;
    ...
    switch (adat.aac.typ) {
        case 0:  // AAC Sequence Header (2 bytes)
            ...
            generate_asc(adat.aac.asc, str_to_audioprof("LC"), str_to_samplerate_idx("44100"), 2);
            if (len >= 4) {
