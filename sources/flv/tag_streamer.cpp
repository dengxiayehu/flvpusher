...
TagStreamerBase::TagStreamerBase(const std::string& dump_path) : m_strm_len(0) {
    if (!dump_path.empty())
            ...
            case FLVParser::NALU: {
                ...
                FOR_VECTOR_ITERATOR(NaluItem*, *vdat.pkt.nalu.dat, it) {
                    ...
                    if (!key_frame) {
                        ...
                        if (nalu_type == 5 /*IDR*/ || nalu_type == 7 /*SPS*/ || nalu_type == 8 /*PPS*/) {
                            key_frame = true;
#ifdef XDEBUG
                            LOGD("KeyFrame detected, nalu type: %d, timestamp: %d", nalu_type,
                                 VALUI24(tag.hdr.timestamp));
#endif
                        }
                    }
                }
                ...
                FOR_VECTOR_ITERATOR(NaluItem*, *vdat.pkt.nalu.dat, it) {
                    // Add nalu_startcode
                    memcpy(dat + offset, nalu_startcode, sizeof(nalu_startcode));
                    offset += 4;
...
int ScriptTagStreamer::process(FLVParser::FLVTag& tag) {
    ...
    byte* p = (byte*)m_mem_holder.alloc(VALUI24(tag.hdr.datasize));
