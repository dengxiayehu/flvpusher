...
#pragma pack(1)
    ...
    /////////////////////////////////////////////////////////////
    enum TagHeaderType { TAG_AUDIO = (byte)0x08, TAG_VIDEO = (byte)0x09, TAG_SCRIPT = (byte)0x12 };
...
public:
    int read_header(FLVHeader& hdr, uint8_t* buf = NULL, uint32_t buf_size = 0);
    int read_tag(FLVTag*& tag, uint8_t* buf = NULL, uint32_t buf_size = 0);
...
private:
    ...
    static int handle_script(amf::AMFData& script, const byte strm[], uint32_t len);
    static int handle_video(FLVVideoTagData& vdat, const byte strm[], uint32_t len);
    static int handle_audio(FLVAudioTagData& adat, const byte strm[], uint32_t len);

    static int parse_avc(const byte*& p, uint32_t len, FLVVideoTagData& vdat);
