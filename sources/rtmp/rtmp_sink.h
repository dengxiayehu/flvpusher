...
public:
    ...
    virtual int send_video(int32_t timestamp, byte* dat, uint32_t length, uint32_t composition_time);
    virtual int send_audio(int32_t timestamp, byte* dat, uint32_t length);

    bool send_rtmp_pkt(int pkttype, uint32_t ts, const byte* buf, uint32_t pktsize);
...
private:
    struct DataInfo {
        ...
        DataInfo() : lts(0), tm_offset(0), need_cfg(true) {
        }
    };
...
private:
    ...
    static int make_avc_dcr_body(byte* buf, const byte* sps, uint32_t sps_len, const byte* pps, uint32_t pps_len);
    static int make_video_body(byte* buf, uint32_t dat_len, bool key_frame, uint32_t composition_time);

    static byte pkttyp2channel(byte typ);

    static int rtmp_check_alloc_array(RTMPPacket** prev_pkt, int* nb_prev_pkt, int channel);
    static int rtmp_packet_create(RTMPPacket* pkt, int channel_id, int type, int timestamp, int size);
    static int rtmp_send_packet(RTMPContext* rt, RTMPPacket* pkt, int track);
    static void rtmp_packet_destroy(RTMPPacket* pkt);
    static int rtmp_packet_write(RTMPContext* rt, RTMPPacket* pkt, int chunk_size, RTMPPacket** prev_pkt_ptr,
                                 int* nb_prev_pkt);
