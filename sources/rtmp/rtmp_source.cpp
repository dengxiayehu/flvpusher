...
RtmpSource::RtmpSource(const std::string& input, MediaSink*& sink)
        : MediaPusher(input, sink), m_rtmp(NULL), m_buffer_time(RTMP_DEF_BUFTIME) {
...
int RtmpSource::prepare() {
    ...
    m_rtmp = RTMP_Alloc();
    if (!m_rtmp) {
        LOGE("RTMP_Alloc() failed for source: \"%s\"", STR(m_input));
        return -1;
    }
    ...
    if (!(ret = RTMP_ParseURL(STR(m_input), &parsed_protocol, &parsed_host, &parsed_port, &parsed_playpath,
                              &parsed_app))) {
        LOGE("Couldn't parse the specified source: \"%s\"", STR(m_input));
        goto out;
    }

    {
        ...
        AVal tcurl = {str, snprintf(str, sizeof(str) - 1, "%s://%.*s:%d/%.*s",
                                    RTMPProtocolStringsLower[parsed_protocol], parsed_host.av_len, parsed_host.av_val,
                                    parsed_port, parsed_app.av_len, parsed_app.av_val)};

        RTMP_SetupStream(m_rtmp, parsed_protocol, &parsed_host, parsed_port, &sockhost, &parsed_playpath, &tcurl, NULL,
                         NULL, &parsed_app, NULL, NULL, 0, NULL, NULL, NULL, 0, 0, TRUE, SOCK_TIMEOUT);
    }

    RTMP_SetBufferMS(m_rtmp, m_buffer_time);

    if (!(ret = RTMP_Connect(m_rtmp, NULL))) {
        LOGE("RTMP_Connect failed for source: \"%s\"", STR(m_input));
        goto out;
    }

    if (!(ret = RTMP_ConnectStream(m_rtmp, 0))) {
        LOGE("RTMP_ConnectStream failed for source: \"%s\"", STR(m_input));
        goto out;
    }

    LOGI("Connect to rtmp source with url \"%s\" ok", STR(m_input));
out:
...
int RtmpSource::disconnect() {
    if (m_rtmp) {
        if (RTMP_IsConnected(m_rtmp)) {
            LOGI("Try to disconnect from source.. (url: %s)", STR(m_input));
        }
...
int RtmpSource::loop() {
        ...
        if (nread > 0) {
            ...
            if (duration > 0) {
                if (m_buffer_time < duration * 1000.0) {
                    ...
                    LOGD("Detected that buffer time is less than duration, resetting to: %ums", m_buffer_time);
                    RTMP_SetBufferMS(m_rtmp, m_buffer_time);
            ...
            if (read_header) {  // Read flv file header first
                ...
                if ((nprocessed = parser.read_header(hdr, (uint8_t*)buf, nread)) < 0) {
            ...
            while (nread > 0) {  // Cover multi-tag in one RTMP_Read()
                ...
                if ((nprocessed = parser.read_tag(tag, (uint8_t*)(buf + offset), nread)) < 0) {
                ...
                int32_t timestamp = (tag->hdr.timestamp_ext << 24) + VALUI24(tag->hdr.timestamp);
                ...
                switch (tag->hdr.typ) {
                    case FLVParser::TAG_VIDEO: {
                        ...
                        if (m_vstrmer->get_strm_length() == 0) {
                            AVCDecorderConfigurationRecord avc_dcr = tag->dat.video.pkt.avc_dcr;
                            SPS sps;
                            GetBitContext gb;
                        ...
                        on_frame(timestamp, m_vstrmer->get_strm(), m_vstrmer->get_strm_length(), 1, composition_time);
                        if (m_sink->send_video(timestamp, m_vstrmer->get_strm(), m_vstrmer->get_strm_length(),
                                               composition_time) < 0) {
                    ...
                    case FLVParser::TAG_AUDIO:
                        ...
                        if (m_astrmer->get_strm_length() == 0) {
                            ...
                            if (parse_asc(asc, profile, samplerate_idx, channel) < 0) {
                                LOGE("Parse asc(%02x %02x) failed", asc.dat[0], asc.dat[1]);
                                break;
                            }
                            m_info.samplerate = atoi(samplerate_idx_to_str(samplerate_idx));
...
#endif
                        on_frame(timestamp, m_astrmer->get_strm(), m_astrmer->get_strm_length(), 0);
                        if (m_sink->send_audio(timestamp, m_astrmer->get_strm(), m_astrmer->get_strm_length()) < 0) {
                    ...
                    case FLVParser::TAG_SCRIPT:
                        ...
                        if (m_sink->type() == MediaSink::RTMP_SINK &&
                            !((RtmpSink*)m_sink)
                                     ->send_rtmp_pkt(RTMP_PACKET_TYPE_INFO, 0, m_sstrmer->get_strm(),
    ...
    } while (!interrupt_cb() && nread > -1 && RTMP_IsConnected(m_rtmp) && !RTMP_IsTimedout(m_rtmp));
