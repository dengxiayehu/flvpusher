...
RtmpSink::RtmpSink(const std::string& flvpath) : MediaSink(flvpath) {
...
int RtmpSink::connect(const std::string& liveurl) {
    ...
    rt->rtmp = RTMP_Alloc();
    if (!rt->rtmp) {
        LOGE("RTMP_Alloc() failed for liveurl: \"%s\"", liveurl.c_str());
        return -1;
    }
    ...
    if (!RTMP_SetupURL(rt->rtmp, const_cast<char*>(liveurl.c_str()))) {
        LOGE("RTMP_SetupURL() failed for liveurl: \"%s\"", liveurl.c_str());
        goto bail;
    }
    ...
    if (!RTMP_Connect(rt->rtmp, NULL)) {
        LOGE("RTMP_Connect failed for liveurl: \"%s\"", liveurl.c_str());
        goto bail;
    };

    if (!RTMP_ConnectStream(rt->rtmp, 0)) {
        LOGE("RTMP_ConnectStream failed for liveurl: \"%s\"", liveurl.c_str());
        goto bail;
    }

    m_url = liveurl;

    LOGI("Connect to rtmp server with url \"%s\" ok", m_url.c_str());
    return 0;
...
int RtmpSink::disconnect() {
    ...
    if (rt->rtmp) {
        if (RTMP_IsConnected(rt->rtmp)) {
            LOGI("Disconnect from rtmp server (url: %s)", m_url.c_str());
        }
...
int RtmpSink::send_video(int32_t timestamp, byte* dat, uint32_t length, uint32_t composition_time) {
    if (m_vparser->process(dat, length) < 0) {
        LOGE("Process video failed");
        return -1;
    }

    byte* buf = (byte*)m_mem_holder.alloc(length + VIDEO_BODY_HEADER_LENGTH + 128 /*Just in case*/);
    byte* cur = buf + VIDEO_PAYLOAD_OFFSET;

    for (uint32_t idx = 0; idx < m_vparser->get_nalu_num(); ++idx) {
        const byte* nalu_dat = m_vparser->get_nalu_data(idx);
        uint32_t nalu_length = m_vparser->get_nalu_length(idx);
    ...
    if (timestamp - m_vinfo.lts < -NEW_STREAM_TIMESTAMP_THESHO) {
        ...
        LOGI("New video stream starts, atm_offset:%d, vtm_offset:%d", m_ainfo.tm_offset, m_vinfo.tm_offset);
#endif
    ...
    if (m_vinfo.need_cfg || m_vparser->sps_pps_changed()) {
        if (m_vparser->is_key_frame()) {
            ...
            int body_len = make_avc_dcr_body(avc_dcr_body, m_vparser->get_sps(), m_vparser->get_sps_length(),
                                             m_vparser->get_pps(), m_vparser->get_pps_length());
            if (!send_rtmp_pkt(RTMP_PACKET_TYPE_VIDEO, timestamp + m_vinfo.tm_offset, avc_dcr_body, body_len)) {
    ...
    int body_len = make_video_body(buf, cur - buf, m_vparser->is_key_frame(), composition_time);
    if (!send_rtmp_pkt(RTMP_PACKET_TYPE_VIDEO, timestamp + m_vinfo.tm_offset, buf, body_len)) {
...
int RtmpSink::make_video_body(byte* buf, uint32_t dat_len, bool key_frame, uint32_t composition_time) {
    uint32_t idx = 0;
...
int RtmpSink::make_avc_dcr_body(byte* buf, const byte* sps, uint32_t sps_len, const byte* pps, uint32_t pps_len) {
    uint32_t idx = 0;
...
#ifdef XDEBUG
    LOGI("[avc_dcr] SPS: %02x %02x %02x %02x ... (%u bytes in total)", sps[0], sps[1], sps[2], sps[3], sps_len);
    LOGI("[avc_dcr] PPS: %02x %02x %02x %02x ... (%u bytes in total)", pps[0], pps[1], pps[2], pps[3], pps_len);
#endif
    return idx;
}
...
int RtmpSink::send_audio(int32_t timestamp, byte* dat, uint32_t length) {
...
#ifdef XDEBUG_TIMESTAMP
            LOGI("New audio stream starts, atm_offset:%d, vtm_offset:%d", m_ainfo.tm_offset, m_vinfo.tm_offset);
#endif
        }

        // NOTE: asc-pkt's timestamp is always 0
        if (!send_rtmp_pkt(RTMP_PACKET_TYPE_AUDIO, timestamp + m_ainfo.tm_offset, asc_body, sizeof(asc_body))) {
    ...
    if (!send_rtmp_pkt(RTMP_PACKET_TYPE_AUDIO, timestamp + m_ainfo.tm_offset, buf, body_len)) {
...
int RtmpSink::make_audio_body(const byte* dat, uint32_t dat_len, byte buf[], uint32_t len) {
    buf[0] = 0xAF;
    buf[1] = 0x01;
...
bool RtmpSink::send_rtmp_pkt(int pkttype, uint32_t ts, const byte* buf, uint32_t pktsize) {
    if (m_flvmuxer.is_opened()) {
        if (m_flvmuxer.write_tag(pkttype, ts, buf, pktsize) < 0) {
            LOGE("Write tag to flv file \"%s\" failed (cont)", m_flvmuxer.get_path());
            // Fall through
        }
    }
    ...
    if (((pkttype == RTMP_PACKET_TYPE_VIDEO || pkttype == RTMP_PACKET_TYPE_AUDIO) && !ts) ||
        ...
        if (rtmp_check_alloc_array(&rt->prev_pkt[1], &rt->nb_prev_pkt[1], channel) < 0)
            return false;
        rt->prev_pkt[1][channel].channel_id = 0;
    }

    if (rtmp_packet_create(&rt->out_pkt, channel, pkttype, ts, pktsize) < 0)
        return false;
...
int RtmpSink::rtmp_check_alloc_array(RTMPPacket** prev_pkt, int* nb_prev_pkt, int channel) {
    int nb_alloc;
    RTMPPacket* ptr;
...
int RtmpSink::rtmp_packet_create(RTMPPacket* pkt, int channel_id, int type, int timestamp, int size) {
    if (size) {
...
int RtmpSink::rtmp_send_packet(RTMPContext* rt, RTMPPacket* pkt, int track) {
    ...
    ret = rtmp_packet_write(rt, pkt, rt->rtmp->m_outChunkSize, &rt->prev_pkt[1], &rt->nb_prev_pkt[1]);
...
int RtmpSink::rtmp_packet_write(RTMPContext* rt, RTMPPacket* pkt, int chunk_size, RTMPPacket** prev_pkt_ptr,
                                int* nb_prev_pkt) {
    ...
    if ((ret = rtmp_check_alloc_array(prev_pkt_ptr, nb_prev_pkt, pkt->channel_id)) < 0)
        return ret;
    prev_pkt = *prev_pkt_ptr;

    use_delta = prev_pkt[pkt->channel_id].channel_id && pkt->extra == prev_pkt[pkt->channel_id].extra &&
