...
MediaPusher::MediaPusher(const std::string& input, MediaSink*& sink)
        : m_input(input), m_sink(sink), m_itime_base((AVRational){1001, 24000}), m_start_time(0) {
}
...
int MediaPusher::on_frame(const int32_t ts, const byte* dat, const uint32_t dat_len, int is_video,
    ...
    if (!m_tspath.empty()) {
        ...
        if (m_tsmuxer.write_frame(ts, dat, dat_len, is_video, composition_time) < 0)
            return -1;
    }
