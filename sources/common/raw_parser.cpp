...
RawParserBase::RawParserBase() : m_raw_len(0) {
}
...
VideoRawParser::VideoRawParser() : m_sps_len(0), m_pps_len(0), m_key_frame(false), m_sps_pps_changed(false) {
}
        ...
        if (startcoede_found) {  // If startcoede found
            ...
            if (dat - nalu_start > 0) {  // If extra startcode founded, ignore it
                ...
                if (nalu_typ == 7 /*SPS*/) {
                    ...
                    if (m_sps_len != 0 &&
                        (m_sps_len != (uint32_t)(dat - nalu_start) || memcmp(m_sps, nalu_start, m_sps_len))) {
                    ...
                    // Check whether sps has changed
                    if (m_pps_len != 0 &&
                        (m_pps_len != (uint32_t)(dat - nalu_start) || memcmp(m_pps, nalu_start, m_pps_len))) {
                        m_sps_pps_changed = true;
                    }
                ...
                m_nalus.push_back(new NaluItem(dat - nalu_start, nalu_start));
            } else {
                ++nalu_ignored;
            }
        } else {
...
#ifdef XDEBUG
    LOGD("Video nalus#: %u, ignored#: %u", m_nalus.size(), nalu_ignored);
    if (m_key_frame) {
        LOGD("m_sps_len=%u, first 4 bytes is: %02x %02x %02x %02x", m_sps_len, m_sps[0], m_sps[1], m_sps[2], m_sps[3]);
        LOGD("m_pps_len=%u, first 4 bytes is: %02x %02x %02x %02x", m_pps_len, m_pps[0], m_pps[1], m_pps[2], m_pps[3]);
    }
    uint32_t nalu_len = 0;
    FOR_VECTOR_ITERATOR(NaluItem*, m_nalus, it) {
        LOGD("length=%u, first 4 bytes is: %02x %02x %02x %02x", (*it)->first, (*it)->second[0], (*it)->second[1],
...
void AudioRawParser::adts_header2asc(const byte adts_header[7], byte asc[2]) {
    byte profile = (adts_header[2] & 0xC0) >> 6;                                   // 2 bits
    byte sample_rate_idx = (adts_header[2] & 0x3C) >> 2;                           // 4 bits
    byte channel = ((adts_header[2] & 0x01) << 2) | (adts_header[3] & 0xC0) >> 6;  // 3 bits

#ifdef XDEBUG
    LOGI("profile=%u, sample_rate_idx=%u, channel=%u", profile, sample_rate_idx, channel);
#endif
