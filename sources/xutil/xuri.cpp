...
Uri::Uri()
        : protocol(NULL), username(NULL), password(NULL), host(NULL), port(80), path(NULL), option(NULL), buffer(NULL) {
}
...
int Uri::parse(const char* str, unsigned char opt_sep) {
    ...
    char *cur = buf, *next;
    next = buf;
    while ((*next >= 'A' && *next <= 'Z') || (*next >= 'a' && *next <= 'z') || (*next >= '0' && *next <= '9') ||
           memchr("+-.", *next, 3))
        next++;
    if (!strncmp(next, "://", 3)) {
        *next = '\0';
...
string Uri::to_string() const {
    return sprintf_("protocol=%s, username=%s, password=%s, host=%s, port=%u, path=%s, option=%s", protocol, username,
                    password, host, port, path, option);
}
