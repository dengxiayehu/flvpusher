...
ssize_t writen(int fd, const void* buf, size_t n) {
    ...
    for (ptr = static_cast<const char*>(buf), to_write = n; to_write > 0; ptr += nwritten, to_write -= nwritten) {
...
ssize_t readn(int fd, void* buf, size_t n) {
    ...
    for (ptr = static_cast<char*>(buf), to_read = n; to_read > 0; ptr += nread, to_read -= nread) {
...
std::string uuid() {
    ...
    CHECK_EXPR_EXEC_RETVAL(!exec_get_str("uuidgen -t", buff, sizeof(buff)), LOGE("try to generate uuid failed"),
                           sprintf_("%d", rand()));
...
std::string time_label() {
    ...
    if (-1 == gettimeofday(&tv, NULL)) {
        ...
        snprintf(time_buf, sizeof(time_buf), "%04d-%02d-%02d-%02d:%02d:%02d.%03ld ", ptm->tm_year + 1900,
                 ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, tv.tv_usec / 1000);
        return time_buf;
    }
}
...
unsigned char* base64_decode(const char* in, unsigned in_size, unsigned& result_size, bool trim_trailing_zeros) {
...
int scandir(void* opaque, const char* path, int (*cb)(void*, const char*)) {
    DIR* dir;
    ...
    while (!ret && (dirent = readdir(dir))) {
        if (!strcmp(dirent->d_name, ".") || !strcmp(dirent->d_name, ".."))
            continue;

        if (cb(opaque, STR(sprintf_("%s%c%s", path, DIRSEP, dirent->d_name))) < 0)
            ret = -1;
    }
...
Mutex::Mutex(MutexType typ) {
    ...
    pthread_mutexattr_settype(&m_attr, typ == NORMAL ? PTHREAD_MUTEX_NORMAL : PTHREAD_MUTEX_RECURSIVE);
...
Condition::Condition(Mutex& m, ConditionType typ) : m_mutex(m) {
    pthread_condattr_init(&m_cond_attr);
    pthread_condattr_setpshared(&m_cond_attr, typ == PRIVATE ? PTHREAD_PROCESS_PRIVATE : PTHREAD_PROCESS_SHARED);
...
status_t Signaler::install(sighandler_t hdl, ...) {
    ...
    va_list ap;
    va_start(ap, hdl);
    while ((signo = va_arg(ap, int)) > 0 && signo != SIGLIST_END) {
...
Thread::Thread(bool detach) : m_tid(-1), m_detach(detach) {
}
...
IOBuffer::IOBuffer() : buffer(NULL), size(0), published(0), consumed(0), min_chunk_size(4096) {
}
...
bool IOBuffer::write_to_stdio(int fd, uint32_t size_, int& sent_amount) {
    bool result = true;
    sent_amount = writen(fd, buffer + consumed, size_ > published - consumed ? published - consumed : size_);
    if (sent_amount < 0) {
...
AutoFileLock::AutoFileLock(const std::string& flock_path, short l_type) : m_flock_path(flock_path) {
    // Make sure the l_type is correct
    assert(l_type == F_WRLCK || l_type == F_RDLCK);

    m_fd = open(STR(m_flock_path), O_RDWR | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
    if (m_fd < 0) {
...
int64_t time_gap(const Time* t1, const Time* t2) {
    assert(t1 && t2);
