...
private:
    ...
    struct conn_info {
        ...
        static int progress(void* p, double dltotal, double dlnow, double ult, double uln);
    };
...
Curl::request* Curl::request::build(request_type type, const char* url,
                                    size_t (*write_cb)(void*, size_t, size_t, void*), void* write_data, long timeout,
                                    const char* range, bool verbose, bool trace_ascii, bool no_signal,
                                    bool follow_location, long max_redirs, bool progress,
                                    size_t (*read_cb)(void*, size_t, size_t, void*), void* read_data,
                                    const char* post_fields, long post_fields_size, char* cookie_file,
                                    long low_speed_limit, long low_speed_time, const char* user_agent) {
...
CurlImpl::glob_info::glob_info() : still_running(0) {
...
#ifdef XDEBUG
                    LOGD("easy(%p) DONE: (url=\"%s\", eff_url=\"%s\", response_code=%d, speed_download=%0.3f "
                         "kbytes/sec, total_time=%0.3f sec) ==> (%d) %s",
                         conn->easy, conn->req->url, conn->req->effective_url, conn->req->response_code,
                         conn->req->speed_download, conn->req->total_time, msg->data.result, conn->error);
#endif
                    --transfers;
                    break;
                }
...
CurlImpl::conn_info::conn_info() : easy(NULL), req(NULL), glob(NULL), trace_ascii(true) {
...
#ifdef XDEBUG
    LOGD("easy(%p) DONE: (url=\"%s\", eff_url=\"%s\", response_code=%d, speed_download=%0.3f kbytes/sec, "
         "total_time=%0.3f sec)",
         easy, req->url, req->effective_url, req->response_code, req->speed_download, req->total_time);
#endif
    return 0;
}
...
void CurlImpl::conn_info::cleanup() {
    if (easy) {
        if (glob) {
            vector<conn_info*>::iterator it = find(glob->connvec.begin(), glob->connvec.end(), this);
...
int CurlImpl::conn_info::trace(CURL* handle, curl_infotype type, char* data, size_t size, void* userp) {
...
int CurlImpl::conn_info::progress(void* p, double dltotal, double dlnow, double ult, double uln) {
#ifdef XDEBUG
...
int CurlImpl::init(long flags) {
    ...
    if (cc != CURLE_OK) {
        LOGE("curl_global_init() failed: %s", curl_easy_strerror(cc));
        return -1;
    }
    return 0;
}
