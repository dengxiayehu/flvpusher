...
int get_amf_associate_array(const byte*& p, uint32_t len, AMFAssociateArray& amfasoarr) {
    ...
    if (idx < amfasoarr.sz || get_amf_obj_end(p, len - (p - savep)) < 0) {
    ...
    list_for_each_entry_safe(amfobj, x, head, list) {
        switch (amfobj->typ) {
            ...
            case AMF_TYPE_DATE: {
                ...
                printf("%s%s %d\n", indent,
                       sprintf_("%02d/%02d/%04d-%02d:%02d:%02d", ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_year + 1900,
...
int put_amf_associate_array(byte*& p, const AMFAssociateArray& amfasoarr) {
    ...
    *p++ = AMF_TYPE_ASSOCIATIVE_ARRAY;
    ...
    if (amfasoarr.arr) {
        bool err = false;
        FOR_VECTOR_ITERATOR(ArrayItem*, *amfasoarr.arr, it) {
            put_amf_string_no_typ(p, (*it)->first->amfstr.str);
