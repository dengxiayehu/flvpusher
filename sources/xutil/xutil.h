...
static constexpr int MaxLine = 4098;
...
inline const char* xstrerror(status_t st) {
    static const char* info[] = {"success",      "error occurred", "invalid param", "internal error",  "logical error",
                                 "system error", "not exists",     "remote error",  "not implemented", "error input"};
...
unsigned char* base64_decode(const char* in, unsigned in_size, unsigned& result_size, bool trim_trailing_zeros = true);
...
#define TAG_KIND_OF(tag, kind) (static_cast<bool>(((tag) & get_tag_mask((kind))) == (kind)))
...
public:
    explicit Condition(Mutex& m, ConditionType typ = PRIVATE);
    ~Condition();
...
#define DECL_THREAD_ROUTINE(Class, func)                                                                         \
    ...
    public:                                                                                                      \
        func##Thread(Class* arg1, void* arg2, bool detach) : xutil::Thread(detach), m_arg1(arg1), m_arg2(arg2) { \
            pthread_attr_t attr;                                                                                 \
            pthread_attr_init(&attr);                                                                            \
            pthread_attr_setdetachstate(&attr, m_detach ? PTHREAD_CREATE_DETACHED : PTHREAD_CREATE_JOINABLE);    \
            pthread_create(&m_tid, &attr, Thread::thread_router, reinterpret_cast<void*>(this));                 \
...
#define CREATE_THREAD_ROUTINE(func, arg, detach) new func##Thread(this, reinterpret_cast<void*>(arg), detach)
...
#define GETIBPOINTER(x) (reinterpret_cast<uint8_t*>((x).buffer + (x).consumed))
...
class AutoFileLock {
public:
    explicit AutoFileLock(const std::string& flock_path, short l_type = F_WRLCK);
    ~AutoFileLock();
