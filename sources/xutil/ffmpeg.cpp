...
const uint8_t static mpeg4audio_channels[8] = {0, 1, 2, 3, 4, 5, 6, 8};

const int static priv_mpeg4audio_sample_rates[16] = {96000, 88200, 64000, 48000, 44100, 32000, 24000,
...
void compute_frame_duration(FormatContext* s, int* pnum, int* pden, Stream* st, CodecParserContext* pc, Packet* pkt) {
    int frame_size;
...
Stream* format_new_stream(FormatContext* s) {
    ...
    if (s->nb_streams >= INT_MAX / sizeof(*streams))
        return NULL;
    streams = (Stream**)realloc(s->streams, (s->nb_streams + 1) * sizeof(*streams));
    if (!streams)
        return NULL;
    s->streams = streams;
...
void priv_set_pts_info(Stream* s, int pts_wrap_bits, unsigned int pts_num, unsigned int pts_den) {
...
Packet* add_to_pktbuf(PacketList** packet_buffer, Packet* pkt, PacketList** plast_pktl) {
...
int read_from_packet_buffer(PacketList** pkt_buffer, PacketList** pkt_buffer_end, Packet* pkt) {
    PacketList* pktl;
    assert(*pkt_buffer);
    pktl = *pkt_buffer;
    *pkt = pktl->pkt;
...
int parse_packet(FormatContext* s, Packet* pkt, int stream_index) {
    ...
    while (size > 0 || (pkt == &flush_pkt && got_output)) {
        ...
        out_pkt.reset();
        len = parser_parse2(st->parser, st->codec, &out_pkt.data, &out_pkt.size, data, size, pkt->pts, pkt->dts,
                            pkt->pos);
...
int parser_parse2(CodecParserContext* s, CodecContext* avctx, uint8_t** poutbuf, int* poutbuf_size, const uint8_t* buf,
...
void compute_pkt_fields(FormatContext* s, Stream* st, CodecParserContext* pc, Packet* pkt) {
    int num, den;
    AVRational duration;
    ...
    if (pkt->duration == 0) {
        ...
        if (den && num) {
            ...
            pkt->duration = av_rescale_rnd(1, num * (int64_t)st->time_base.den, den * (int64_t)st->time_base.num,
                                           AV_ROUND_DOWN);
        }
    }

    if (pkt->pts != -1 || pkt->dts != -1 || pkt->duration) {
        if (st->start_time == -1 || (pkt->pts >= 0 && st->start_time > pkt->pts)) {
...
int aac_parse(CodecParserContext* s1, CodecContext* avctx, const uint8_t** poutbuf, int* poutbuf_size,
...
int h264_parse(CodecParserContext* s, CodecContext* avctx, const uint8_t** poutbuf, int* poutbuf_size,
               const uint8_t* buf, int buf_size) {
    H264Context* h = (H264Context*)s->priv_data;
...
int aac_sync(uint64_t state, AACParseContext* hdr_info, int* need_next_header, int* new_frame_start) {
...
int combine_frame(ParseContext* pc, int next, const uint8_t** buf, int* buf_size) {
    if (pc->overread) {
#ifdef DEBUG
        LOGD("overread %d, state:%X next:%d index:%d o_index:%d", pc->overread, pc->state, next, pc->index,
             pc->overread_index);
    ...
    if (next == END_NOT_FOUND) {
        void* new_buffer = realloc(pc->buffer, *buf_size + pc->index + INPUT_BUFFER_PADDING_SIZE);
    ...
    *buf_size = pc->overread_index = pc->index + next;
    ...
    // Append to buffer
    if (pc->index) {
        void* new_buffer = realloc(pc->buffer, next + pc->index + INPUT_BUFFER_PADDING_SIZE);
        if (!new_buffer) {
...
int parse_nal_units(CodecParserContext* s, CodecContext* avctx, const uint8_t* const buf, int buf_size) {
    H264Context* h = (H264Context*)s->priv_data;
    ...
    for (;;) {
        ...
        ptr = h264_decode_nal(h, buf + buf_index, &dst_length, &consumed, src_length);
        if (!ptr || dst_length < 0)
            break;
...
int find_start_code(const uint8_t* buf, int buf_size, int buf_index, int next_avc) {
    uint32_t state = -1;
    const uint8_t *p = buf + buf_index, *end = buf + next_avc + 1;
    int i;
...
uint8_t* h264_decode_nal(H264Context* h, const uint8_t* src, int* dst_length, int* consumed, int length) {
    int i, si, di;
    uint8_t* dst;
    int bufidx;
...
#define STARTCODE_TEST                                          \
    ...
    if (h->rbsp_buffer_size[bufidx] < wanted_size) {
        uint8_t* p = (uint8_t*)realloc(h->rbsp_buffer[bufidx], wanted_size);
        if (!p) {
...
int get_avc_nalsize(H264Context* h, const uint8_t* buf, int buf_size, int* buf_index) {
    int i, nalsize = 0;
...
void estimate_timings_from_pts(FormatContext* ic, off_t old_offset) {
    ...
    for (i = 0; i < ic->nb_streams; ++i) {
        st = ic->streams[i];
        if (st->start_time == -1 && st->codec->codec_type != MEDIA_TYPE_UNKNOWN)
...
unsigned int choose_output(FormatContext* ic) {
    ...
    for (unsigned i = 0; i < ic->nb_streams; ++i) {
        Stream* st = ic->streams[i];
        int opts = av_rescale_q(st->cur_dts, st->time_base, AV_TIME_BASE_Q);
        if (opts < opts_min) {
            opts_min = opts;
            index = i;
        }
...
int interleave_add_packet(FormatContext* s, Packet* pkt, int (*compare)(FormatContext*, Packet*, Packet*)) {
    ...
    if (*next_point) {
        if (compare(s, &s->packet_buffer_end->pkt, pkt)) {
            while (*next_point && !compare(s, &(*next_point)->pkt, pkt))
...
int interleave_compare_dts(FormatContext* s, Packet* next, Packet* pkt) {
    Stream* st = s->streams[pkt->stream_index];
...
int read_frame_internal(FormatContext* s, Packet* pkt) {
    ...
    while (!got_packet && !s->parse_queue) {
        ...
        if (cur_pkt.pts != -1 && cur_pkt.dts != -1 && cur_pkt.pts < cur_pkt.dts) {
            LOGW("Invalid timestamps stream=%d, pts=%lld, dts=%lld, size=%d", cur_pkt.stream_index, cur_pkt.pts,
                 cur_pkt.dts, cur_pkt.size);
        }
#ifdef XDEBUG
        LOGD("read_packet stream=%d, pts=%lld, dts=%lld, size=%d, duration=%d", cur_pkt.stream_index, cur_pkt.pts,
             cur_pkt.dts, cur_pkt.size, cur_pkt.duration);
#endif
...
out:
#ifdef XDEBUG
    LOGD("After format_find_stream_info() pos: %lld", ic->file->cursor());
#endif
    return ret;
}
...
#ifdef XDEBUG
    LOGI("demuxer -> stream: %d pkt_pts:%lld, pkt_dts:%lld size:%d duration:%d pos:%lld", pkt.stream_index, pkt.pts,
         pkt.dts, pkt.size, pkt.duration, pkt.pos);
#endif
...
#ifdef XDEBUG
    LOGI("demuxer+ffmpeg -> stream: %d pkt_pts:%lld, pkt_dts:%lld size:%d duration:%d pos:%lld", pkt.stream_index,
...
#ifdef XDEBUG
    LOGI("muxer <- stream: %d pkt_pts:%lld pkt_dts:%lld size:%d duration:%d pos:%lld", pkt->stream_index, pkt->pts,
         pkt->dts, pkt->size, pkt->duration, pkt->pos);
#endif
...
int interleave_packet_per_dts(FormatContext* s, Packet* out, Packet* pkt, int flush) {
    PacketList* pktl;
    ...
    if (s->max_interleave_delta > 0 && s->packet_buffer && !flush &&
        ...
        int64_t top_dts = av_rescale_q(top_pkt->dts, s->streams[top_pkt->stream_index]->time_base, AV_TIME_BASE_Q);
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            last_dts = av_rescale_q(last->pkt.dts, s->streams[i]->time_base, AV_TIME_BASE_Q);
...
int write_trailer(FormatContext* s) {
    ...
    for (;;) {
        ...
        if (s->cb) {
            ...
            Frame frame;
            if (frame.make_frame(opkt.pts, opkt.data, opkt.size, true) < 0 || s->cb(s->opaque, &frame, is_video) < 0) {
...
const uint8_t* priv_find_start_code(const uint8_t* p, const uint8_t* end, uint32_t* state) {
    int i;
