...
AddressPort::AddressPort(const string& ip, const uint16_t port) : m_ip(ip), m_port(port) {
}
...
int get_local_address_from_sockfd(int sockfd, AddressPort& ap) {
    ...
    int ret = getsockname(sockfd, (struct sockaddr*)&addrin, &addrin_len);
    if (ret < 0) {
    ...
    if (!inet_ntop(AF_INET, &addrin.sin_addr, ip, sizeof(ip))) {
...
int Socket::increate_buffer_to(int buf_opt_name, int requested_size) {
    ...
    while (requested_size < cur_size) {
        ...
        if (setsockopt(get_sockfd(), SOL_SOCKET, buf_opt_name, (char*)&requested_size, size_size) >= 0) {
...
int Socket::get_buffer_size(int buf_opt_name) {
    ...
    if (getsockopt(get_sockfd(), SOL_SOCKET, buf_opt_name, (char*)&cur_size, &size_size) < 0) {
...
int Socket::sock_buf_fill(SockBuf* sb) {
    ...
    for (;;) {
        ...
        nread = ::recv(sb->sb_socket, sb->sb_start + sb->sb_size, nread, MSG_NOSIGNAL);
        if (nread != -1)
            sb->sb_size += nread;
        else {
            if (errno == EINTR)
...
int Socket::sock_buf_send(SockBuf* sb, const uint8_t* buf, int size, struct sockaddr_in* remote) {
    if (remote)
        return ::sendto(sb->sb_socket, buf, size, MSG_NOSIGNAL, (struct sockaddr*)remote, sizeof(*remote));
    else
...
int Socket::connect(const AddressPort& ap) {
    ...
    if (::connect(get_sockfd(), (struct sockaddr*)&m_connaddr, sizeof(struct sockaddr)) < 0) {
...
int Tcp::open(AddressPort& ap) {
    ...
    int opt = 1;
    setsockopt(get_sockfd(), SOL_SOCKET, SO_REUSEADDR, (char*)&opt, sizeof(opt));
    struct timeval tv = {SockRecvTimeout, 0};
    setsockopt(get_sockfd(), SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(tv));
    ...
    if (::bind(get_sockfd(), (struct sockaddr*)&m_bindaddr, sizeof(struct sockaddr)) < 0) {
...
int Udp::open(AddressPort& ap) {
    ...
    setsockopt(get_sockfd(), SOL_SOCKET, SO_REUSEADDR, (char*)&opt, sizeof(opt));
    opt = 255;
    setsockopt(get_sockfd(), IPPROTO_IP, IP_MULTICAST_TTL, (char*)&opt, sizeof(opt));
    ...
    if (IN_MULTICAST(ntohl(inet_addr(bind_ip)))) {
        opt = 1;
        setsockopt(get_sockfd(), IPPROTO_IP, IP_MULTICAST_LOOP, (char*)&opt, sizeof(opt));
        ...
        setsockopt(get_sockfd(), IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&multicast_addr, sizeof(multicast_addr));
    }

    if (inet_addr(bind_ip) == INADDR_BROADCAST) {
        opt = 1;
        setsockopt(get_sockfd(), SOL_SOCKET, SO_BROADCAST, (char*)&opt, sizeof(opt));
    }
    return 0;
}
...
NetAddressList::NetAddressList(const char* hostname) : m_num_addresses(0), m_address_array(NULL) {
    ...
    if (!host || host->h_length != 4 || !host->h_addr_list)
        return;
    ...
    while (p) {
        if (p->ai_addrlen < 4)
            continue;
        m_address_array[i++] = new NetAddress((const uint8_t*)&(((struct sockaddr_in*)p->ai_addr)->sin_addr.s_addr), 4);
        p = p->ai_next;
    }
...
void NetAddressList::clean() {
    ...
    delete[] m_address_array;
    m_address_array = NULL;
}

NetAddress const* NetAddressList::first_address() const {
    if (m_num_addresses == 0)
        return NULL;
    return m_address_array[0];
}

NetAddressList::Iterator::Iterator(const NetAddressList& address_list) : m_address_list(address_list), m_next_index(0) {
}

const NetAddress* NetAddressList::Iterator::next_address() {
    if (m_next_index >= m_address_list.num_addresses())
        return NULL;
...
bool is_multicast_address(NetAddressBits address) {
    ...
    return address_in_network_order > 0xE00000FF && address_in_network_order <= 0xEFFFFFFF;
}
