...
#define CHECK_EXPR_EXEC(expr, cmd) \
    do {                           \
        if (expr) {                \
            cmd;                   \
        }                          \
    } while (0)
#define CHECK_EXPR_EXEC_RET(expr, cmd) \
    do {                               \
        if (expr) {                    \
            cmd;                       \
            return;                    \
...
#define CHECK_EXPR_EXEC_RETVAL(expr, cmd, retval) \
    do {                                          \
        if (expr) {                               \
            cmd;                                  \
            return retval;                        \
...
#define CHECK_EXPR_EXEC_GOTO(expr, cmd, label) \
    do {                                       \
        if (expr) {                            \
            cmd;                               \
            goto label;                        \
...
#define htonll(x)                                                                                                     \
    ((uint64_t)(((((uint64_t)(x)) & 0xff00000000000000LL) >> 56) | ((((uint64_t)(x)) & 0x00ff000000000000LL) >> 40) | \
...
#define REVERSE_BYTES(bytes_arr, n)                    \
    do {                                               \
...
#define VALUI24(x) ((uint32_t)((x)[2] << 16) + (uint32_t)((x)[1] << 8) + (uint32_t)(x)[0])
...
#define MAKE_TAG8(a, b, c, d, e, f, g, h)                                                                              \
