...
struct CodecParser {
    ...
    int (*parser_parse)(CodecParserContext* s, CodecContext* avctx, const uint8_t** poutbuf, int* poutbuf_size,
...
enum AVRounding {
    ...
    AV_ROUND_PASS_MINMAX = 8192,  ///< Flag to pass INT64_MIN/MAX through instead of rescaling, this avoids special
                                  ///< cases for AV_NOPTS_VALUE
};
...
struct AACParseContext {
    ...
    int frame_size;
    int header_size;
    int (*sync)(uint64_t state, struct AACParseContext* hdr_info, int* need_next_header, int* new_frame_start);
    int channels;
    int sample_rate;
    int bit_rate;
    int samples;
...
int interleave_packet_per_dts(FormatContext* ic, Packet* out, Packet* pkt, int flush);
