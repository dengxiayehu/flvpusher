...
#define STARTCODE3(nalu) (nalu[0] == 0 && nalu[1] == 0 && nalu[2] == 1)
// Startcode: |0x00 0x00 0x00 0x01|
#define STARTCODE4(nalu) (nalu[0] == 0 && nalu[1] == 0 && nalu[2] == 0 && nalu[3] == 1)
...
public:
    ...
    int make_frame(int32_t ts, byte* dat, uint32_t dat_len, bool reuse_dat, uint32_t composition_time = 0);
    void clear();
    const int32_t& get_dts() const {
        return m_dts;
    }
    const uint32_t& get_composition_time() const {
        return m_composition_time;
    }
    byte* get_data() const {
        return m_dat;
    }
    const uint32_t& get_data_length() const {
        return m_dat_len;
    }
    void set_data(byte* dat) {
        m_dat = dat;
    }
...
class BitrateCalc {
public:
    BitrateCalc() : m_bits(0), m_bitrate(0), m_tm_last(0) {
    }
...
class FPSCalc {
public:
    FPSCalc() : m_frame_num(0), m_fps(0), m_tm_last(0) {
    }
