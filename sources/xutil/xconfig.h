...
typedef enum { INTEGER = 0, BOOL, STRING, ENUM, UNKNOWN } ConfigType;
...
#define GET_CONFIG_INT(c, x)                  \
    do {                                      \
        char* s = (c)->get_config(#x);        \
        if (!s) {                             \
            LOGE("No " #x " in config file"); \
            assert(0);                        \
        }                                     \
        x = atoi(s);                          \
    } while (0)
#define DECL_GET_CONFIG_INT(c, x) \
    int x;                        \
    GET_CONFIG_INT(c, x)

#define GET_CONFIG_BOOL(c, x)                 \
    do {                                      \
        char* s = (c)->get_config(#x);        \
        if (!s) {                             \
            LOGE("No " #x " in config file"); \
        ...
        if (s && !strncasecmp(s, "true", 4))  \
            x = true;                         \
        else                                  \
            x = false;                        \
    } while (0)
#define DECL_GET_CONFIG_BOOL(c, x) \
    bool x;                        \
    GET_CONFIG_BOOL(c, x)

#define GET_CONFIG_STRING(c, x)               \
    do {                                      \
        char* s = (c)->get_config(#x);        \
        if (!s) {                             \
            LOGE("No " #x " in config file"); \
...
#define DECL_GET_CONFIG_STRING(c, x) \
    std::string x;                   \
    GET_CONFIG_STRING(c, x)
