...
private:
    struct ConfigItem {
        ...
        ConfigItem();
        ConfigItem(char* name_, char* value_, char* typestr, char* def_, char* range_, char* note_);
        ~ConfigItem();
...
ConfigImpl::ConfigItem::ConfigItem() : name(NULL), value(NULL), type(UNKNOWN), def(NULL), range(NULL), note(NULL) {
}

ConfigImpl::ConfigItem::ConfigItem(char* name_, char* value_, char* typestr, char* def_, char* range_, char* note_)
        : name(strdup_(name_)),
          value(strdup_(value_)),
          type(str2type(typestr)),
          def(strdup_(def_)),
          range(strdup_(range_)),
          note(strdup_(note_)) {
}
...
bool ConfigImpl::ConfigItem::is_valid() const {
    switch (type) {
        case INTEGER:
            return check_integer(value, range);
        case BOOL:
            return check_bool(value);
        case STRING:
            return check_string(value);
        case ENUM:
            return check_enum(value, range);
        case UNKNOWN:
        default:
            return !def && !range ? true : false;
    }
}
...
again:
    ...
    if (*range) {
        ...
        *pdst = '\0';
        ...
        if ((*range == ',' && *++range) || (range = skip_blank(range), *range))
            goto again;
    }
    bool retval = false;
    for (int i = 0; i < n; ++i) {
...
ConfigImpl::ConfigImpl() : m_thrd(NULL), m_watch_variable(NULL) {
    m_ctl[0] = m_ctl[1] = -1;
}
...
int ConfigImpl::set_config(const char* conf_name, const char* conf_value, const char* note) {
    ...
    if (!m_cb_map.empty() && strcmp(conf_value, item->value) && m_reg.find(conf_name) != m_reg.end()) {
...
int ConfigImpl::add_config(const char* conf_name, const char* conf_value, const char* note, ConfigType type,
                           const char* def, const char* range) {
    AutoLock _l(m_mutex);
    ...
    ConfigItem* item = new ConfigItem((char*)conf_name, (char*)conf_value, (char*)ConfigItem::type2str(type),
                                      (char*)def, (char*)range, (char*)note);
    if (!item->is_valid()) {
        SAFE_DELETE(item);
        return -1;
    }
...
int ConfigImpl::load(const char* config_path) {
    ...
    while (file.read_line(line, sizeof(line))) {
        ...
        *pdst = '\0';  // value done
        ...
        def[0] = '\0';
        range[0] = '\0';
        if (!(pdst = strchr(pline, '#')))
            goto item_done;
        if (!(pline = strchr(pdst + 1, '<')))
            goto skip;
        ...
        *pdst = '\0';  // type done
        ...
        if (!strncasecmp(type, "INTEGER", 7) || !strncasecmp(type, "ENUM", 4)) {
    ...
    item_done:
        item = new ConfigItem(name, value, type[0] ? type : NULL, def[0] ? def : NULL, range[0] ? range : NULL,
                              note[0] ? note : NULL);
        if (!item->is_valid())
            goto skip;
        if (has_config(name)) {
            if (!m_cb_map.empty() && m_reg.find(name) != m_reg.end() && strcmp(m_map[name]->value, value)) {
                FOR_MAP(m_cb_map, ConfigUpdateCB, void*, it)
...
#define EVENT_HEADER_SIZE (sizeof(struct inotify_event))
    ...
    fd_set rfd;
    struct timeval tv;
    char buff[BUFSIZ] __attribute__((aligned(__alignof__(EVENT_HEADER_SIZE))));
    int n;
    ...
    while (!*m_watch_variable) {
        if (wd < 0) {
            ...
            if ((wd = inotify_add_watch(fd, STR(m_path), IN_IGNORED | IN_CLOSE_WRITE | IN_MOVE_SELF | IN_MOVE)) < 0) {
            ...
            while (off < n) {
                ...
                if (ev->wd == wd) {
                    ...
                    if (ev->mask & IN_IGNORED) {
                        ...
                        if (ev->mask & (IN_MOVE_SELF | IN_MOVE))
                            sync();
