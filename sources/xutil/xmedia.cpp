...
Frame::Frame() : m_dts(-1), m_composition_time(0), m_dat(NULL), m_dat_len(0), m_capacity(0) {
}

int Frame::make_frame(int32_t ts, byte* dat, uint32_t dat_len, bool reuse_dat, uint32_t composition_time) {
    if (!reuse_dat) {
...
int generate_asc(AudioSpecificConfig& asc, uint8_t profile, uint8_t sample_rate_idx, uint8_t channel) {
    asc.dat[0] = profile << 3;
...
int parse_asc(const AudioSpecificConfig& asc, uint8_t& profile, uint8_t& sample_rate_idx, uint8_t& channel) {
    return parse_asc(asc.dat, 2, profile, sample_rate_idx, channel);
}
int parse_asc(const uint8_t* buf, int len, uint8_t& profile, uint8_t& sample_rate_idx, uint8_t& channel) {
    /* profile: 5bits
...
int generate_adts_header(const AudioSpecificConfig& asc, uint32_t aac_len, byte adts_hdr[7]) {
    uint8_t profile, sample_rate_idx, channel;
...
int generate_adts_header(const uint8_t asc_buf[2], uint32_t aac_len, byte adts_hdr[7]) {
...
static struct {
    const char* sprofile;
    int profile;
} audio_profs[] = {{"MAIN", 1},   {"LC", 2},      {"SSR", 3}, {"LTP", 4},       {"HE_AAC", 5},
                   {"ER_LC", 17}, {"ER_LTP", 19}, {"LD", 23}, {"DRM_ER_LC", 27}};
...
static int samplerate[] = {96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050,
...
static const uint8_t ff_zigzag_direct[64] = {0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32, 25, 18, 11, 4,  5,
...
static void decode_scaling_list(GetBitContext* gb, uint8_t* factors, int size, const uint8_t* jvt_list,
...
static const uint8_t default_scaling4[2][16] = {{6, 13, 20, 28, 13, 20, 28, 32, 20, 28, 32, 37, 28, 32, 37, 42},
...
static void decode_scaling_matrices(GetBitContext* gb, SPS* sps, PPS* pps, int is_sps, uint8_t (*scaling_matrix4)[16],
                                    uint8_t (*scaling_matrix8)[64]) {
    int fallback_sps = !is_sps && sps->scaling_matrix_present;
    const uint8_t* fallback[4] = {fallback_sps ? sps->scaling_matrix4[0] : default_scaling4[0],
...
int h264_decode_sps(GetBitContext* gb, SPS* sps) {
    ...
    if (sps->poc_type == 0) {
        ...
        if ((unsigned)sps->poc_cycle_length >= NELEM(sps->offset_for_ref_frame)) {
    ...
    if (sps->ref_frame_count > H264_MAX_PICTURE_COUNT - 2 || sps->ref_frame_count > 16) {
