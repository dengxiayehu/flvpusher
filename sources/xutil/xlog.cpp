...
static const char* color_level[] = {COLOR_FORMAT_LEVEL_DEBUG, COLOR_FORMAT_LEVEL_INFO, COLOR_FORMAT_LEVEL_WARN,
...
status_t log_add_dst(const char* logfile, log_level lvl, int flgs) {
    ...
    int fd = open(logfile, O_WRONLY | O_CREAT | O_NOCTTY | (flgs & LOG_TRUNC ? O_TRUNC : O_APPEND), 0666);
    if (fd < 0) {
...
int log_print(const char* curfile, const int lineno, const log_level lvl, const char* fmt, ...) {
    ...
    for (log_t* l = log; l; l = l->next) {
        ...
        char buf[MaxLine];
        int ret = snprintf(buf, sizeof(buf), "%s%s%s[%s:%d] %s ", color_level[lvl], time_buf, tid_buf,
                           STR(basename_(curfile)), lineno, lvl_name[lvl]);
...
const int k_exception_signals[] = {SIGSEGV, SIGABRT, SIGFPE, SIGILL, SIGBUS, -1};
...
static int sig_crashes_setup() {
    ...
    _signal[SIGSEGV] = "SIGSEGV";
    _signal[SIGABRT] = "SIGABRT";
    _signal[SIGFPE] = "SIGFPE";
    _signal[SIGILL] = "SIGILL";
    _signal[SIGBUS] = "SIGBUS";
    _signal[SIGQUIT] = "SIGQUIT";
    _signal[SIGPIPE] = "SIGPIPE";

    if (sigaltstack(NULL, &stack) == -1 || !stack.ss_sp || stack.ss_size < k_sig_stack_size) {
...
static void fault_handler_sigaction(int signum, siginfo_t* si, void* misc) {
    ...
    len += snprintf(stack_buffer + len, SEGMENT_FAULT_STRING_LEN - len,
                    ...
                    "!errno: %d, code: %d\n",
                    pid, cmdline, _signal[si->si_signo], si->si_signo, si->si_addr, si->si_errno, si->si_code);
    ...
    if (!symbols) {
        ...
        for (i = 0; i < nptrs; ++i) {
            ...
            if (sbuffer[0] == '\0' || strstr(sbuffer, "??") != NULL)
                ...
                len += snprintf(stack_buffer + len, SEGMENT_FAULT_STRING_LEN - len, "%s", sbuffer);
    ...
    snprintf(stack_buffer, SEGMENT_FAULT_STRING_LEN,
             ...
             (long int)syscall(SYS_gettid), pid, timestr);
    for (;;) {
        LOGW("%s", stack_buffer);
        sleep_(30 * 1000);
    }
...
static int addr2line(int pid, const char* library, const char* addr, char* buffer) {
    ...
    FILE* fpc = popen(cmdline, "r");
    if (fpc == nullptr) {
        return 3;
    }

    memset(buffer, 0, 512);
    if (fread(buffer, 512, 1, fpc) < 0) {
        pclose(fpc);
        return 4;
    }
    pclose(fpc);
...
static int gettimestr(char* buf) {
    ...
    if (-1 == gettimeofday(&tv, NULL)) {
        ...
        sprintf(buf, "%04d-%02d-%02d-%02d:%02d:%02d.%03ld ", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday,
