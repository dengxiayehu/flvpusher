...
bool is_valid_vod_m3u8(const string& filename) {
    if (is_file(filename)) {
        ...
        if (is_valid_m3u8(GETIBPOINTER(*iobuf), GETAVAILABLEBYTESCOUNT(*iobuf))) {
            ...
            if (!strncmp((const char*)GETIBPOINTER(*iobuf), m3u8_endline, endline_len))
                return true;
        }
    }
    return false;
...
bool is_valid_m3u8(const uint8_t* buf, size_t size) {
    ...
    while (size--) {
        static const char* const ext[] = {"TARGETDURATION", "MEDIA-SEQUENCE", "KEY",           "ALLOW-CACHE",
                                          "ENDLIST",        "STREAM-INF",     "DISCONTINUITY", "VERSION"};
