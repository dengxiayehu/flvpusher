...
string HLSPusher::Segment::to_string() const {
    return sprintf_("Segment -- sequence=%d, duration=%d, size=%llu, url=%s, key_path=%s", sequence, duration,
                    (long long unsigned)size, url, key_path);
}
...
int HLSPusher::HLSStream::download_segment_key(StreamSys* sys, Segment* seg) {
    ...
    if (read_content_from_url(curl_hls_timeout, curl_verbose, true, seg->key_path, iobuf.get()) < 0) {
...
int HLSPusher::HLSStream::download_segment_data(StreamSys* sys, Segment* seg, int cur_stream) {
    ...
    if (sys->bandwidth > 0 && bandwidth > 0) {
        ...
        if (estimated > seg->duration) {
            LOGW("Downloading Segment %d predicted to take %ds, which exceeds its length (%ds)", seg->sequence,
                 estimated, seg->duration);
        }
    }

    uint64_t start = get_time_now();
    if (download(sys, seg) < 0) {
        LOGE("Downloading Segment %d from stream %d failed", seg->sequence, cur_stream);
        return -1;
    }
    duration_ = get_time_now() - start;

    LOGD("Downloaded Segment(%s) from stream %d", STR(seg->to_string()), cur_stream);
...
int HLSPusher::HLSStream::download(StreamSys* sys, Segment* seg) {
    ...
    if (read_content_from_url(curl_hls_timeout, curl_verbose, true, seg->url, seg->iobuf) < 0) {
...
string HLSPusher::HLSStream::to_string() const {
    return sprintf_(
            "HLSStream -- id=%d, version=%d, sequence=%d, duration=%d, max_segment_length=%d, bandwidth=%llu, "
            "size=%llu, %d# segments, url=%s, cache=%s, current_key_path=%s",
            id, version, sequence, duration, max_segment_length, (long long unsigned)bandwidth,
            (long long unsigned)size, segments.size(), url, cache ? "true" : "false", current_key_path);
}

HLSPusher::StreamSys::StreamSys(HLSPusher* pusher_)
        : conf(pusher_->m_conf),
          m3u8(strdup_(STR(pusher_->m_input))),
          bandwidth(0),
          cache(false),
          meta(false),
          live(true),
...
int HLSPusher::StreamSys::reload_playlist() {
    ...
    for (int n = 0; n < count; ++n) {
        ...
        if (!hls_old) {
            ...
            LOGW("New HLS stream appended (id=%d, bandwidth=%llu ignored)", hls_new->id,
                 (long long unsigned)hls_new->bandwidth);
            continue;
        } else if (update_playlist(hls_new, hls_old, &stream_appended) < 0) {
            LOGW("Failed updating HLS stream (id=%d, bandwidth=%llu)", hls_new->id,
                 (long long unsigned)hls_new->bandwidth);
        }
        SAFE_DELETE(hls_new);
    }
...
int HLSPusher::StreamSys::get_http_live_meta_playlist(vector<HLSStream*>& streams) {
    ...
    for (int i = 0; i < get_hls_count(); ++i) {
        ...
        if (read_m3u8_from_url(this, dst->url, iobuf.get()) < 0) {
            ...
            err = parse_m3u8(this, streams, GETIBPOINTER(*iobuf), GETAVAILABLEBYTESCOUNT(*iobuf));
        }
    }
    return err;
}
...
int HLSPusher::StreamSys::update_playlist(HLSStream* hls_new, HLSStream* hls_old, bool* stream_appended) {
    ...
    LOGD("Updating hls stream (program-id=%d, bandwidth=%llu) has %d segments", hls_new->id,
         (long long unsigned)hls_new->bandwidth, count);
    ...
    for (int n = 0; n < count; ++n) {
        ...
        if (seg) {
            ...
            if ((p->sequence != seg->sequence) || (p->duration != seg->duration) || (strcmp(p->url, seg->url))) {
                ...
                if ((p->key_path || p->key_loaded) && seg->is_downloaded()) {
                    SAFE_DELETE(seg->iobuf);
                }
            ...
            if (l->sequence + 1 != p->sequence) {
                LOGW("Gap in sequence numbers found: new=%d expected %d", p->sequence, l->sequence + 1);
            }
...
HLSPusher::HLSStream* HLSPusher::StreamSys::find_hls(HLSStream* hls_new) {
    ...
    for (int n = 0; n < count; ++n) {
        ...
        if (hls) {
            if ((hls->id == hls_new->id) && ((hls->bandwidth == hls_new->bandwidth) || (hls_new->bandwidth == 0)) &&
...
HLSPusher::HLSPusher(const string& input, MediaSink*& sink, Config* conf)
        : MediaPusher(input, sink), m_conf(conf), m_sys(NULL) {
}
...
int HLSPusher::prepare() {
    ...
    if (parse_m3u8(m_sys, m_sys->streams, GETIBPOINTER(*iobuf), GETAVAILABLEBYTESCOUNT(*iobuf)) < 0) {
...
int HLSPusher::loop() {
    ...
    HLSStream* hls = m_sys->get_hls(m_sys->playback.stream);
    assert(hls);

    while (!interrupt_cb()) {
        if (m_sys->download.segment <= m_sys->playback.segment) {  // Quick check
            AutoLock _l(m_sys->download.mutex);
            while ((m_sys->download.segment <= m_sys->playback.segment) && !interrupt_cb()) {
        ...
        hls->mutex.lock();
        Segment* seg = hls->get_segment(m_sys->playback.segment);
        hls->mutex.unlock();
        ...
        END
                ...
                skip : AutoLock _l(m_sys->download.mutex);
        if (++m_sys->playback.segment && (!m_sys->live && m_sys->playback.segment >= hls->get_segment_count())) {
            break;
        }
...
int HLSPusher::live_segment(Segment* seg) {
    ...
    snprintf(tempts, sizeof(tempts), "flvpusher-%s-XXXXXX", STR(basename_(seg->url)));
    if (mkstemp(tempts) < 0) {
        LOGE("mkstemp failed: %s", ERRNOMSG);
        SAFE_DELETE(seg->iobuf);
        return -1;
    }
    File::flush_content(tempts, GETIBPOINTER(*seg->iobuf), GETAVAILABLEBYTESCOUNT(*seg->iobuf), "wb");

    // Parse ts file and read video & audio frames
    int ret = 0;
    auto_ptr<TSParser> ts_parser(new TSParser);
    if (ts_parser->set_file(tempts, true) < 0) {
        LOGE("Parse segment \"%s\" failed", seg->url);
        ret = -1;
        goto out;
    }
...
int HLSPusher::parsed_frame_cb(void* opaque, xmedia::Frame* f, int is_video) {
    ...
    if (is_video) {
        if (obj->m_sink->send_video(dts, f->get_data(), f->get_data_length()) < 0) {
            LOGE("Send video data to %sserver failed", STR(obj->m_sink->type_str()));
            ret = -1;
        }
    } else {
        if (obj->m_sink->send_audio(dts, f->get_data(), f->get_data_length()) < 0) {
            LOGE("Send video data to %sserver failed", STR(obj->m_sink->type_str()));
            ret = -1;
        }
    }
...
int HLSPusher::read_content_from_url(int timeout, bool verbose, bool trace_ascii, const char* url, IOBuffer* iobuf) {
    auto_ptr<Curl> curl(new Curl);
    Curl::request* req = Curl::request::build(Curl::GET, url, Curl::write_cb, iobuf, timeout, NULL, verbose,
                                              trace_ascii, true, true);
    if (!req) {
        LOGE("Build GET for url \"%s\" failed", url);
        return -1;
    }
    int ret = 0;
    if (curl->perform(req, NULL) < 0 || req->response_code != 200) {
        LOGE("read_content_from_url(%s) failed (response_code=%d)", url, req->response_code);
        ret = -1;
    }
...
int HLSPusher::read_m3u8_from_url(StreamSys* sys, const char* url, IOBuffer* iobuf) {
    ...
    return read_content_from_url(curl_hls_timeout, curl_verbose, curl_trace_ascii, url, iobuf);
}

int HLSPusher::parse_m3u8(StreamSys* sys, vector<HLSStream*>& streams, uint8_t* buffer, const ssize_t len) {
    assert(sys && buffer);
                    ...
                    if (*uri == '#') {
                        ...
                        if (hls) {
                            ...
                            if (read_m3u8_from_url(sys, hls->url, iobuf.get()) < 0) {
                                ...
                                iobuf->read_from_byte(0);
                                err = parse_m3u8(sys, streams, GETIBPOINTER(*iobuf), GETAVAILABLEBYTESCOUNT(*iobuf));
                            }
...
int HLSPusher::parse_stream_information(StreamSys* sys, vector<HLSStream*>& streams, HLSStream** hls, char* read,
                                        const char* uri) {
    int id;
    uint64_t bw;
    char* attr;
    ...
    LOGD("Bandwidth adaptation detected (program-id=%d, bandwidth=%llu).", id, (unsigned long long)bw);
...
char* HLSPusher::parse_attributes(const char* line, const char* attr) {
    ...
    begin = p;
    do {
        if (!strncasecmp(begin, attr, strlen(attr)) && begin[strlen(attr)] == '=') {
            p = strchr(begin, ',');
            begin += strlen(attr) + 1;
...
int HLSPusher::parse_media_sequence(StreamSys* sys, HLSStream* hls, char* read) {
    ...
    if (hls->sequence > 0) {
        if (sys->live) {
            ...
            if ((last_segment->sequence < sequence) && (sequence - last_segment->sequence > 1)) {
                LOGE("EXT-X-MEDIA-SEQUENCE gap in playlist (new=%d, old=%d)", sequence, last_segment->sequence);
            }
        } else {
            LOGE("EXT-X-MEDIA-SEQUENCE already present in playlist (new=%d, old=%d)", sequence, hls->sequence);
        }
    }
...
int HLSPusher::choose_segment(StreamSys* sys, const int current) {
    ...
    while ((i >= 0) && (i < count)) {
        ...
        if (seg->duration > hls->duration) {
            LOGW("EXTINF:%d duration is larger than EXT-X-TARGETDURATION:%d", seg->duration, hls->duration);
        }
    ...
    while (!interrupt_cb()) {
        ...
        if (now >= playlist.wakeup) {
            if (reload_playlist() < 0) {
                ...
                if (playlist.tries == 1)
                    wait = 0.5;
                else if (playlist.tries == 2)
                    wait = 1;
                else if (playlist.tries == 3)
                    wait = 1.5;
...
void* HLSPusher::StreamSys::hls_routine(void* arg) {
    ...
    while (!interrupt_cb()) {
        ...
        if ((!live && (playback.segment < (count - 6))) || (download.segment >= count)) {
            AutoLock _l(download.mutex);
            while (((download.segment - playback.segment > 6) || (download.segment >= count)) && !interrupt_cb()) {
                download.wait.wait();
        ...
        if (seg && hls->download_segment_data(this, seg, download.stream) < 0) {
            if (interrupt_cb())
                break;
            // Fall through
        }
