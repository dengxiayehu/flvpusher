...
private:
    ...
    static int read_content_from_url(int timeout, bool verbose, bool trace_ascii, const char* url,
                                     xutil::IOBuffer* iobuf);
    static int read_m3u8_from_url(StreamSys* sys, const char* url, xutil::IOBuffer* iobuf);
    static int parse_m3u8(StreamSys* sys, std::vector<HLSStream*>& streams, uint8_t* buffer, const ssize_t len);
    static int parse_stream_information(StreamSys* sys, std::vector<HLSStream*>& streams, HLSStream** hls, char* read,
                                        const char* uri);
