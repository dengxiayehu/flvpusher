...
HLSSegmenter::HLSSegmenter(const string& hls_playlist, const int hls_time)
        : m_hls_playlist(hls_playlist), m_hls_time(hls_time), m_mf(UNSUPPORTED) {
    bzero(&u, sizeof(u));
}
...
int HLSSegmenter::set_file(const string& filename, bool loop) {
    ...
    int ret = 0;
    if (end_with(filename, ".mp4") || end_with(filename, ".3gp") || end_with(filename, ".3gpp")) {
        m_mf = MP4;
        ...
        LOGE("Not support file:\"%s\" for hls-segmenter", STR(filename));
        ret = -1;
        goto out;
    }
    ...
    END
        ...
        if (!loop) {
            ...
            if (!info_file->open(sprintf_("%s%c%s", STR(dirname_(m_hls_playlist)), DIRSEP, DEFAULT_HLS_INFO_FILE),
                                 "wb"))
                return -1;
...
int HLSSegmenter::create_m3u8(bool create_ts) {
    ...
    if (m_mf == MP4) {
        ...
        if (!seek_file->write_buffer((uint8_t*)rs, sizeof(rs))) {
            LOGE("Write seek file \"%s\" failed", seek_file->get_path());
            return -1;
        }
        while (!interrupt_cb() && !u.mp4.parser->mp4_read_packet(u.mp4.parser->m_mp4->stream, pkt)) {
            int is_video = is_h264_video(pkt->data, pkt->size);
            int is_key = is_h264_key(pkt->data, pkt->size) || is_aac_audio(pkt->data, pkt->size);
            if (is_video)
                info->duration = (double)(pkt->pts - info->end_pts) * tb.num / tb.den;
            int64_t end_pts = m_hls_time * (int64_t)AV_TIME_BASE * info->number;
            if (is_video && is_key && av_compare_ts(pkt->pts - info->start_pts, tb, end_pts, AV_TIME_BASE_Q) >= 0) {
                if (create_ts)
                    SAFE_DELETE(tsmuxer);
            ...
            if (create_ts)
                tsmuxer->write_frame(pkt->dts, pkt->data, pkt->size, is_video, pkt->pts - pkt->dts);
            memcpy(rs, u.mp4.parser->m_status, sizeof(rs));
            SAFE_FREE(pkt->data);
        }
        if (create_ts)
        ...
        while (!interrupt_cb() && !u.flv.parser->eof()) {
            ...
            int32_t pkt_dts = (tag->hdr.timestamp_ext << 24) + VALUI24(tag->hdr.timestamp);
            ...
            BEGIN
            bool is_key = is_h264_key(pkt_data, pkt_size) || is_aac_audio(pkt_data, pkt_size);
            if (is_video)
                info->duration = (double)(pkt_pts - info->end_pts) * tb.num / tb.den;
            int64_t end_pts = m_hls_time * (int64_t)AV_TIME_BASE * info->number;
            if (is_video && is_key && av_compare_ts(pkt_pts - info->start_pts, tb, end_pts, AV_TIME_BASE_Q) >= 0) {
                if (create_ts)
                    SAFE_DELETE(tsmuxer);
                info->segments.push_back((HLSSegment){filename, info->duration});
                if (!seek_file->write_buffer((uint8_t*)rs, sizeof(rs))) {
            ...
            if (create_ts)
                tsmuxer->write_frame(pkt_dts, pkt_data, pkt_size, is_video, pkt_pts - pkt_dts);
            memcpy(rs, u.flv.parser->m_status, sizeof(rs));
            END done : u.flv.parser->free_tag(tag);
        }
        if (create_ts)
            SAFE_DELETE(tsmuxer);
        info->segments.push_back((HLSSegment){filename, info->duration});
    ...
    FOR_VECTOR_ITERATOR(HLSSegment, info->segments, it) {
        n = snprintf(buf, sizeof(buf) - 1,
                     "#EXTINF:%f,\n"
                     "%s\n",
                     it->duration, STR(basename_(it->filename)));
        pl_file->write_buffer((uint8_t*)buf, n);
    }
...
int HLSSegmenter::create_segment(uint32_t idx) {
    ...
    if (rename(STR(segment_tmp), STR(segment_path)) != 0) {
        LOGE("Rename segment_tmp \"%s\" to \"%s\" failed: %s", STR(segment_tmp), STR(segment_path), ERRNOMSG);
...
const std::string HLSSegmenter::get_seek_filename() const {
    return sprintf_("%s/%s.seek", STR(dirname_(m_hls_playlist)), STR(basename_(m_hls_playlist)));
}
...
int HLSSegmenter::create_segment(const std::string& req_segment) {
    ...
    string segment_lock_file(sprintf_("%s.lock", STR(req_segment)));
    BEGIN
    ...
    if (!is_file(req_segment)) {  // need to check again, if one process already
        ...
        if (strncmp(p - hls_uri_seperator_len, HLS_URI_SEPERATOR, hls_uri_seperator_len) != 0) {
        ...
        auto_ptr<HLSSegmenter> hls_segmenter(
                new HLSSegmenter(sprintf_("%.*s.m3u8", p - STR(req_segment), STR(req_segment)), hls_time));
        if (hls_segmenter->set_file(media_file) < 0) {
            LOGE("HLSSegmenter load file \"%s\" failed", STR(media_file));
            return -1;
        }
        hls_segmenter->create_segment(ts_index);

        LOGD("Generate \"%s\" done (%dms used)", STR(req_segment), get_time_now() - generate_start_time);
    }
    END
...
int HLSSegmenter::access_m3u8(const std::string& req_m3u8) {
    ...
    File info_file;
    if (!info_file.open(sprintf_("%s%c%s", STR(dir), DIRSEP, DEFAULT_HLS_INFO_FILE), "rb+")) {
        return -1;
    }
