...
bool TSParser::eof() const {
    return m_file.is_opened() ? m_file.eof() || m_file.cursor() >= m_file.size() : true;
}
...
int TSParser::ts_init(TSContext*& ts, xfile::File* file) {
    ...
    if (!ic) {
        LOGE("calloc for FormatContext failed: %s", ERRNOMSG);
        return -1;
    }
    ...
    if (ic->iformat->priv_data_size > 0) {
        ...
        ;
        if (!ic->priv_data) {
            LOGE("calloc for ic->iformat->priv_data failed: %s", ERRNOMSG);
            return -1;
        }
    }
    ...
    if (!file->read_buffer(buf, sizeof(buf))) {
        if (sizeof(buf) > (size_t)file->size()) {
            LOGE("Too small the ts file is (size: %ld)", (long)file->size());
        }
        return -1;
    }
    ts->raw_packet_size = get_packet_size(buf, sizeof(buf));
    if (ts->raw_packet_size <= 0) {
        LOGE("Could not detect TS packet size (%d)", ts->raw_packet_size);
        return -1;
    }
    return 0;
}
...
int TSParser::analyze(const uint8_t* buf, int size, int packet_size, int* index) {
...
#ifdef XDEBUG
    LOGD("score: %d, dvhs_score: %d, fec_score: %d", score, dvhs_score, fec_score);
#endif
...
TSParser::TSFilter* TSParser::ts_open_filter(TSContext* ts, unsigned int pid, TSFilterType type) {
    TSFilter* filter;
    ...
    if (pid >= NB_PID_MAX || ts->pids[pid]) {
        LOGE("pid(0x%x) too large or filter already attached", pid);
        return NULL;
    }
    filter = (TSFilter*)calloc(1, sizeof(*filter));
    if (!filter) {
        LOGE("calloc for filter(pid=0x%x) failed: %s", pid, ERRNOMSG);
        return NULL;
    }
    // Record it
    ts->pids[pid] = filter;
...
TSParser::TSFilter* TSParser::ts_open_section_filter(TSContext* ts, unsigned int pid, SectionCallback* section_cb,
                                                     void* opaque, int check_crc) {
    TSFilter* filter;
    TSSectionFilter* sec;
    ...
    if (!sec->section_buf) {
        LOGE("malloc for section_buf(pid=0x%x) failed: %s", pid, ERRNOMSG);
        SAFE_FREE(filter);
        return NULL;
    }
    return filter;
...
void TSParser::pat_cb(TSFilter* filter, const uint8_t* section, int section_len) {
    ...
    for (;;) {
        ...
        if (sid == 0x0000) {
            ...
            if (fil) {
                if (fil->type != TS_SECTION || fil->pid != pmt_pid || fil->u.section_filter.section_cb != pmt_cb) {
...
int TSParser::handle_packet(TSContext* ts, const uint8_t* packet) {
    TSFilter* tss;
    int len, error_indicator, pid, cc, expected_cc, cc_ok, afc, is_start, is_discontinuity, has_adaptation, has_payload;
    const uint8_t *p, *p_end;
    off_t pos;
    ...
    has_payload = afc & 1;
    is_discontinuity = has_adaptation && packet[4] != 0 &&  // packet[4]: adaption_field_length
    ...
    tss->last_cc = cc;
    if (!cc_ok) {
        LOGE("Continuity check failed for pid:0x%x expected %d got %d", pid, expected_cc, cc);
        return -1;
    }
...
#ifdef XDEBUG
            LOGI("Filter with pid 0x%x update pcr: %lld (90Hz)", tss->pid, tss->last_pcr / 300);
#endif
        }
 ...
 * (-1) if not available */
int TSParser::parse_pcr(int64_t* ppcr_high, int* ppcr_low, const uint8_t* packet) {
    int afc, len, flags;
    const uint8_t* p;
    unsigned int v;
...
void TSParser::write_section_data(TSContext* ts, TSFilter* tss1, const uint8_t* buf, int buf_size, int is_start) {
...
int TSParser::parse_section_header(SectionHeader* h, const uint8_t** pp, const uint8_t* p_end) {
    int val;
...
void TSParser::pmt_cb(TSFilter* filter, const uint8_t* section, int section_len) {
    TSContext* ts = (TSContext*)filter->u.section_filter.opaque;
...
#ifdef XDEBUG
    LOGD("PMT: sid=0x%x sec_num=%d/%d", h->id, h->sec_num, h->last_sec_num);
#endif
    ...
    for (;;) {
        ...
        if (ts->pids[pid] && ts->pids[pid]->type == TS_PES) {
            ...
            LOGE("Not supported stream_type: 0x%x", stream_type);
            return;
        }
...
TSParser::Program* TSParser::get_program(TSContext* ts, unsigned int programid) {
    for (unsigned i = 0; i < ts->nb_prg; ++i) {
        if (ts->prg[i].id == programid) {
            return &ts->prg[i];
        }
...
void TSParser::add_pid_to_pmt(TSContext* ts, unsigned int programid, unsigned int pid) {
    Program* p = get_program(ts, programid);
    if (!p) {
        LOGE("Program with id: %u not found", programid);
        return;
    }

    if (p->nb_pids >= MAX_PIDS_PER_PROGRAM) {
        LOGE("Too many pids for program(id: %u)", p->id);
        return;
    }
    p->pids[p->nb_pids++] = pid;
}

TSParser::PESContext* TSParser::add_pes_stream(TSContext* ts, int pid, int pcr_pid) {
    TSFilter* tss;
    PESContext* pes;
...
TSParser::TSFilter* TSParser::ts_open_pes_filter(TSContext* ts, unsigned int pid, PESCallback* pes_cb, void* opaque) {
    TSFilter* filter;
    TSPESFilter* pes;
...
int TSParser::ts_push_data(TSFilter* filter, const uint8_t* buf, int buf_size, int is_start, off_t pos) {
    PESContext* pes = (PESContext*)filter->u.pes_filter.opaque;
    TSContext* ts = pes->ts;
    const uint8_t* p;
    int len, code;
            ...
            case TS_HEADER:
                ...
                if (pes->data_index == PES_START_SIZE) {
                    ...
                    if (pes->header[0] == 0x00 && pes->header[1] == 0x00 && pes->header[2] == 0x01) {
                        ...
                        pes->total_size = ntohs(*(uint16_t*)(pes->header + 4));
                        ...
                        if (!pes->data) {
                            LOGE("Alloc for pes->data failed: %s", ERRNOMSG);
                            return -1;
                        }
            ...
            case TS_PESHEADER_FILL:
                ...
                if (pes->data_index == pes->pes_header_size) {
                    ...
                    if (pes->stream_type == 0x12 && buf_size > 0) {
                        LOGE("Not supported pes stream_type: 0x%x", pes->stream_type);
                        return -1;
                    }
...
TSParser::TSFilter* TSParser::ts_open_pcr_filter(TSContext* ts, unsigned int pid) {
...
void TSParser::new_pes_packet(PESContext* pes, Packet* pkt) {
    ...
    if (!p) {
        LOGE("realloc for pkt->data failed: %s", ERRNOMSG);
    } else {
        pkt->reset();
...
int64_t TSParser::parse_pes_pts(const uint8_t* buf) {
    return (int64_t)(*buf & 0x0e) << 29 | (ntohs(*(uint16_t*)(buf + 1)) >> 1) << 15 | ntohs(*(uint16_t*)(buf + 3)) >> 1;
}

void TSParser::sdt_cb(TSFilter* filter, const uint8_t* section, int section_len) {
    TSContext* ts = (TSContext*)filter->u.section_filter.opaque;
    ;
    SectionHeader h1, *h = &h1;
    const uint8_t *p, *p_end, *desc_list_end, *desc_end;
    int onid, val, sid, running_status, desc_list_len, desc_tag, desc_len, service_type;
...
#ifdef XDEBUG
                        LOGI("service_name: %s, service_provider: %s", name, provider_name);
#endif
                    }
                    SAFE_FREE(name);
...
TSParser::Program* TSParser::new_program(TSContext* ts, unsigned int programid, unsigned int running_status) {
    Program* p = get_program(ts, programid);
    if (!p) {
        p = (Program*)realloc(ts->prg, (ts->nb_prg + 1) * sizeof(*ts->prg));
        if (!p) {
            LOGE("realloc for Program failed: %s", ERRNOMSG);
            ts->nb_prg = 0;
            return NULL;
        }
...
int TSParser::ts_set_stream_info(Stream* st, PESContext* pes, uint32_t stream_type) {
...
#ifdef XDEBUG
    LOGI("stream=%d stream_type=%x pid=%x", st->index, pes->stream_type, pes->pid);
#endif
...
void TSParser::ts_find_stream_type(Stream* st, uint32_t stream_type, const StreamType* types) {
