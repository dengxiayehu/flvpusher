...
public:
    ...
    int write_frame(const int32_t ts, const uint8_t* dat, const uint32_t dat_len, int is_video,
...
private:
    ...
    static TSService* ts_add_service(TSWrite* ts, int sid, const char* provider_name, const char* service_name);
    ...
    static void ts_write_pes(FormatContext* s, Stream* st, const uint8_t* payload, int payload_size, int64_t pts,
                             int64_t dts, int key);
    ...
    static int ts_write_section1(TSSection* s, int tid, int id, int version, int sec_num, int last_sec_num,
                                 uint8_t* buf, int len);
