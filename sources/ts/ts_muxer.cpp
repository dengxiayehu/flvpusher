...
static uint32_t crc32(const uint32_t* ctx, uint32_t crc, const uint8_t* buffer, size_t length) {
...
TSMuxer::TSMuxer() : m_ic(NULL), m_file(NULL) {
...
int TSMuxer::init(AVRational itime_base) {
    ...
    service = ts_add_service(ts, ts->service_id, provider_name, service_name);
    ...
    for (unsigned i = 0; i < m_ic->nb_streams; ++i) {
        ...
        if (st->codec->codec_type == MEDIA_TYPE_VIDEO && service->pcr_pid == 0x1fff) {
    ...
    if (ts->mux_rate > 1) {
        service->pcr_packet_period = (ts->mux_rate * ts->pcr_period) / (TS_PACKET_SIZE * 8 * 1000);
        ts->sdt_packet_period = (ts->mux_rate * SDT_RETRANS_TIME) / (TS_PACKET_SIZE * 8 * 1000);
        ts->pat_packet_period = (ts->mux_rate * PAT_RETRANS_TIME) / (TS_PACKET_SIZE * 8 * 1000);
...
#ifdef XDEBUG
    {
        ...
        snprintf(tmp + off, sizeof(tmp) - off - 1, "pcr every %d pkts, sdt every %d pkts, pat/pmt every %d pkts",
                 service->pcr_packet_period, ts->sdt_packet_period, ts->pat_packet_period);
        LOGD(tmp);
    }
#endif
...
TSMuxer::TSService* TSMuxer::ts_add_service(TSWrite* ts, int sid, const char* provider_name, const char* service_name) {
    TSService* service;
    ...
    if (!service->provider_name || !service->name)
        goto bail;
    ts->services = (TSService**)realloc(ts->services, (ts->nb_services + 1) * sizeof(TSService*));
...
int TSMuxer::write_frame(const int32_t ts, const uint8_t* dat, const uint32_t dat_len, int is_video,
                         const uint32_t composition_time) {
    Packet pkt1, *pkt = &pkt1;
    pkt->data = (uint8_t*)malloc(dat_len * sizeof(uint8_t));
    if (!pkt->data) {
        LOGE("malloc for pkt's data failed:%s", ERRNOMSG);
        return -1;
    }
...
int TSMuxer::ts_write_packet_internal(FormatContext* s, Packet* pkt) {
    ...
    if (st->codec->codec_id == CODEC_ID_H264) {
        ...
        LOGE("stream codec_id:%d not supported", st->codec->codec_id);
        return -1;
    }
    ...
    if (pkt->dts != -1) {
        for (unsigned i = 0; i < s->nb_streams; ++i) {
            ...
            if (ts_st2->payload_size && (ts_st2->payload_dts == -1 || dts - ts_st2->payload_dts > delay / 2)) {
                ts_write_pes(s, st2, ts_st2->payload, ts_st2->payload_size, ts_st2->payload_pts, ts_st2->payload_dts,
    ...
    if (ts_st->payload_size && ts_st->payload_size + size > ts->pes_payload_size) {
        ts_write_pes(s, st, ts_st->payload, ts_st->payload_size, ts_st->payload_pts, ts_st->payload_dts,
    ...
    if (st->codec->codec_type != MEDIA_TYPE_AUDIO || size > ts->pes_payload_size) {
        ...
        ts_write_pes(s, st, buf, size, pts, dts, flags & AV_PKT_FLAG_KEY);
        SAFE_FREE(data);
        return 0;
    }
...
void TSMuxer::ts_write_pes(FormatContext* s, Stream* st, const uint8_t* payload, int payload_size, int64_t pts,
                           int64_t dts, int key) {
    TSWriteStream* ts_st = (TSWriteStream*)st->priv_data;
    TSWrite* ts = (TSWrite*)s->priv_data;
    ...
    while (payload_size > 0) {
        ...
        if (ts_st->pid == ts_st->service->pcr_pid) {
            ...
            if (ts_st->service->pcr_packet_count >= ts_st->service->pcr_packet_period) {
                ts_st->service->pcr_packet_count = 0;
                write_pcr = 1;
            }
        }

        if (ts->mux_rate > 1 && dts != -1 && (dts - get_pcr(ts, s->file) / 300) > delay) {
        ...
        if (is_start) {
            ...
            *q++ = 0x01;
            ...
            if (st->codec->codec_type == MEDIA_TYPE_VIDEO)
                *q++ = 0xe0;
            else if (st->codec->codec_type == MEDIA_TYPE_AUDIO && st->codec->codec_id == CODEC_ID_AAC)
                *q++ = 0xc0;
            header_len = 0;
            flags = 0;
            if (pts != -1) {
...
int64_t TSMuxer::get_pcr(const TSWrite* ts, xfile::File* f) {
    return av_rescale(f->cursor() + 11, 8 * PCR_TIME_BASE, ts->mux_rate) + ts->first_pcr;
}
...
void TSMuxer::ts_prefix_m2ts_header(FormatContext* s) {
    ...
    if (ts->m2ts_mode) {
        ...
        s->file->write_buffer((unsigned char*)&tp_extra_header, sizeof(tp_extra_header));
    }
}
    ...
    *q++ = 0xff;
    for (i = 0; i < ts->nb_services; i++) {
        ...
        *q++ = 0x01;  // Digital television service
        ...
        val = (running_status << 13) | (free_ca_mode << 12) | (q - desc_list_len_ptr - 2);
        desc_list_len_ptr[0] = val >> 8;
        desc_list_len_ptr[1] = val;
    }
    ts_write_section1(&ts->sdt, SDT_TID, ts->tsid, ts->tables_version, 0, 0, data, q - data);
}
...
void TSMuxer::ts_write_pat(FormatContext* s) {
    ...
    ts_write_section1(&ts->pat, PAT_TID, ts->tsid, ts->tables_version, 0, 0, data, q - data);
}
...
int TSMuxer::ts_write_pmt(FormatContext* s, TSService* service) {
    ...
    ts_write_section1(&service->pmt, PMT_TID, service->sid, ts->tables_version, 0, 0, data, q - data);
    return 0;
}

int TSMuxer::ts_write_section1(TSSection* s, int tid, int id, int version, int sec_num, int last_sec_num, uint8_t* buf,
                               int len) {
