...
private:
    ...
    typedef int PESCallback(TSFilter* f, const uint8_t* buf, int len, int is_start, off_t pos);
    struct TSPESFilter {
        PESCallback* pes_cb;
        void* opaque;
    };
...
private:
    ...
    static void pat_cb(TSFilter* filter, const uint8_t* section, int section_len);
    static void pmt_cb(TSFilter* filter, const uint8_t* section, int section_len);
    static void sdt_cb(TSFilter* filter, const uint8_t* section, int section_len);

    static TSFilter* ts_open_filter(TSContext* ts, unsigned int pid, TSFilterType type);
    static TSFilter* ts_open_section_filter(TSContext* ts, unsigned int pid, SectionCallback* section_cb, void* opaque,
                                            int check_crc);
    static TSFilter* ts_open_pes_filter(TSContext* ts, unsigned int pid, PESCallback* pes_cb, void* opaque);
    ...
    static void write_section_data(TSContext* ts, TSFilter* tssl, const uint8_t* buf, int buf_size, int is_start);
    ...
    static void add_pid_to_pmt(TSContext* ts, unsigned int programid, unsigned int pid);
    ...
    static int ts_push_data(TSFilter* filter, const uint8_t* buf, int buf_size, int is_start, off_t pos);
    ...
    static Program* new_program(TSContext* ts, unsigned int programid, unsigned int running_status);

    static int ts_set_stream_info(Stream* st, PESContext* pes, uint32_t stream_type);

    static void ts_find_stream_type(Stream* st, uint32_t stream_type, const StreamType* types);
