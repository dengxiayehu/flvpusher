...
static void parse_one_byte_extension_header(struct RTPHeader* header, const uint8_t* ptr_rtp_data_extension_end,
                                            const uint8_t* ptr);
static uint8_t parse_padding_bytes(const uint8_t* ptr_rtp_data_extension_end, const uint8_t* ptr);

int parse_rtp_header(const uint8_t* pkt, int pkt_len, RTPHeaderRaw** hdr, const uint8_t** payload,
                     unsigned* payload_len, RTPHeader* rtp_header) {
...
#ifdef XDEBUG
    LOGD("Payload:%p payload_len:%4u M:%d pt:%u seq:%u timestamp:%u ssrc:0x%x num_CSRCs:%u padding_length:%u "
         "header_length:%u",
         *payload, *payload_len, rtp_header->marker_bit, rtp_header->payload_type, rtp_header->sequence_number,
         rtp_header->timestamp, rtp_header->ssrc, rtp_header->num_CSRCs, rtp_header->padding_length,
         rtp_header->header_length);
#endif
    return 0;
}
...
static const char* frame_type_str(const FrameType frame_type) {
    return frame_type == kFrameKey ? "key" : frame_type == kFrameDelta ? "delta" : "unknown";
}

Packet::Packet(const uint8_t* ptr, const uint32_t size, uint16_t seq_num, uint32_t timestamp, bool marker_bit,
               FrameType ft, bool current_packet_begins_frame, bool first_packet)
        : m_size_bytes(size),
          m_seq_num(seq_num),
          m_timestamp(timestamp),
          m_marker_bit(marker_bit),
          m_frame_type(ft),
          m_current_packet_begins_frame(current_packet_begins_frame),
...
const std::string Packet::to_string() const {
    return sprintf_("payload:%p payload_len:%4u M:%d seq:%u timestamp:%u frame_type:%s", m_data_ptr, m_size_bytes,
                    m_marker_bit, m_seq_num, m_timestamp, frame_type_str(m_frame_type));
}

FrameBuffer::FrameBuffer()
        : m_frame_type(kFrameUnknown),
          m_empty_seq_num_high(-1),
          m_empty_seq_num_low(-1),
          m_timestamp(0),
          m_state(kStateEmpty),
          m_complete(false) {
}
...
FrameBufferEnum FrameBuffer::insert_packet(const Packet* pkt, uint64_t now) {
    ...
    if (pkt->first_packet() || (m_frame_type == kFrameUnknown && pkt->frame_type() != kFrameUnknown))
...
void FrameBuffer::set_state(FrameBufferStateEnum state) {
    ...
    switch (state) {
        ...
        case kStateComplete:
            assert(get_state() == kStateEmpty || get_state() == kStateIncomplete);
            break;
...
void FrameBuffer::update_complete_session() {
    if (MAP_VAL(m_packets.begin())->first_packet() && MAP_VAL(--m_packets.end())->marker_bit()) {
        if (m_packets.size() == 1)
            m_complete = true;
        else {
            bool complete_session = true;
            for (PacketList::iterator it = m_packets.begin(), prev_it = it++; it != m_packets.end(); prev_it = it++) {
...
int FrameMap::recycle_frames_until_key_frame(bool& key_frame_found, FrameList& free_frames) {
    int drop_count = 0;
    ...
    while (!empty()) {
        ...
        LOGI("Recycling: type=%s, low_seq_num=%u, high_seq_num=%u", frame_type_str(frame->frame_type()),
...
int FrameMap::cleanup_old_or_empty_frames(DecodingState* ds, FrameList& free_frames) {
    ...
    while (!empty()) {
        ...
        if (oldest_frame->get_state() == kStateEmpty && size() > 1) {
...
DecodingState::DecodingState() : m_seq_num(0), m_timestamp(0), m_in_initial_state(true) {
}
...
bool DecodingState::update_empty_frame(const FrameBuffer* frame) {
    bool empty_packet = frame->get_high_seq_num() == frame->get_low_seq_num();

    if (m_in_initial_state && empty_packet) {
        // Drop empty packets as long as we are in the initial state
        return true;
    }
    if ((empty_packet && continuous_seq_num((uint16_t)frame->get_high_seq_num())) || continuous_frame(frame)) {
...
FrameBufferEnum Receiver::insert_packet(const Packet* pkt, CompleteFrameProc cb, void* client_data) {
    ...
    switch (buffer_return) {
        ...
        case kIncomplete:
            if (frame->get_state() == kStateEmpty && m_last_decoded_state.update_empty_frame(frame)) {
                frame->reset();
                ret = kNoError;
            } else if (first) {
                ret = kFirstPacket;
...
bool Receiver::is_continuous(const FrameBuffer* frame) {
    ...
    for (FrameMap::Iterator it = m_decodable_frames.begin(); it != m_decodable_frames.end(); ++it) {
...
FrameBufferEnum Receiver::get_frame(const Packet* pkt, FrameBuffer** frame) {
    ...
    *frame = NULL;
    ...
    if (m_last_decoded_state.is_old_packet(pkt)) {
        ...
        if (m_num_consecutive_old_packets >= kMaxConsecutiveOldPackets) {
            LOGW("Too many consecutive_old_packets(%u) reached", kMaxConsecutiveOldPackets);
            flush();
    ...
    *frame = m_incomplete_frames.find_frame(pkt->timestamp());
    if (*frame) {
#ifdef XDEBUG
        LOGD("This packet(seq_num=%u, timestamp=%u) belongs to a incomplete frame(%p)", pkt->seq_num(),
             pkt->timestamp(), *frame);
#endif
        return kNoError;
    }
    *frame = m_decodable_frames.find_frame(pkt->timestamp());
    if (*frame) {
#ifdef XDEBUG
        LOGD("This packet(seq_num=%u, timestamp=%u) belongs to a decodable frame(%p)", pkt->seq_num(), pkt->timestamp(),
             *frame);
#endif
        return kNoError;
    }
...
bool Receiver::recycle_frames_until_key_frame() {
    ...
    dropped_frames += m_incomplete_frames.recycle_frames_until_key_frame(key_frame_found, m_free_frames);
    if (!dropped_frames)
        dropped_frames += m_decodable_frames.recycle_frames_until_key_frame(key_frame_found, m_free_frames);
...
void Receiver::find_and_insert_continuous_frames(const FrameBuffer* new_frame) {
    ...
    for (FrameMap::Iterator it = m_incomplete_frames.begin(); it != m_incomplete_frames.end(); ++it) {
...
bool Receiver::is_continuous_in_state(const FrameBuffer* frame, const DecodingState* ds) {
    ...
    if (frame->get_state() == kStateComplete && ds->continuous_frame(frame))
        return true;
    return false;
}

void Receiver::cleanup_old_or_empty_frames() {
    m_drop_count += m_decodable_frames.cleanup_old_or_empty_frames(&m_last_decoded_state, m_free_frames);
    m_drop_count += m_incomplete_frames.cleanup_old_or_empty_frames(&m_last_decoded_state, m_free_frames);
}
...
bool Receiver::next_maybe_incomplete_timestamp(uint32_t* timestamp) {
    ...
    if (m_decodable_frames.empty() && m_incomplete_frames.size() <= 1 && oldest_frame->get_state() == kStateIncomplete)
        return false;

    if (m_last_decoded_state.in_initial_state() && oldest_frame->frame_type() != kFrameKey)
        return false;
