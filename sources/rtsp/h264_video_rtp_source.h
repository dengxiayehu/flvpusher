...
public:
    ...
    ~SPropRecord() {
        SAFE_FREE(m_s_prop_bytes);
    }

    unsigned& s_prop_length() {
        return m_s_prop_length;
    }
    unsigned char*& s_prop_bytes() {
        return m_s_prop_bytes;
    }
...
public:
    H264VideoRTPSource(TaskScheduler* scheduler, RtpInterface* interface, unsigned char rtp_payload_format,
                       unsigned rtp_timestamp_frequency, const char* s_prop_parm_str = NULL, void* opaque = NULL);
