...
SDPAttribute::SDPAttribute(char const* str_value, bool value_is_hexadecimal)
        : m_str_value(strdup(str_value)), m_str_value_to_lower(NULL), m_value_is_hexadecimal(value_is_hexadecimal) {
    if (!m_str_value) {
        m_int_value = 1;
    } else {
        ...
        for (int i = 0; i < str_size - 1; ++i)
            m_str_value_to_lower[i] = tolower(m_str_value[i]);
...
int MediaSubsession::parse_sdp_attr_rtpmap(const char* sdp_line) {
    ...
    if (sscanf(sdp_line, "a=rtpmap: %u %[^/]/%u/%u", &rtpmap_payload_format, codec_name, &rtp_timestamp_frequency,
               &num_channels) == 4 ||
        sscanf(sdp_line, "a=rtpmap: %u %[^/]/%u", &rtpmap_payload_format, codec_name, &rtp_timestamp_frequency) == 3 ||
        sscanf(sdp_line, "a=rtpmap: %u %s", &rtpmap_payload_format, codec_name) == 2) {
        ret = 0;
        if (rtpmap_payload_format == m_rtp_payload_format) {
            for (char* p = codec_name; *p; ++p)
                *p = toupper(*p);
            SAFE_FREE(m_codec_name);
            m_codec_name = strdup(codec_name);
...
static int parse_range_attr(const char* sdp_line, char*& abs_start_time, char*& abs_end_time) {
...
int MediaSubsession::parse_sdp_attr_fmtp(const char* sdp_line) {
    do {
        if (strncmp(sdp_line, "a=fmtp:", 7) != 0) {
            break;
        }
        sdp_line += 7;
        while (isdigit(*sdp_line))
            ++sdp_line;
        ++sdp_line;
...
int MediaSubsession::parse_sdp_attr_framerate(const char* sdp_line) {
    ...
    float frate;
    int rate;
    if (sscanf(sdp_line, "a=framerate: %f", &frate) == 1 || sscanf(sdp_line, "a=framerate:%f", &frate) == 1) {
        ret = 0;
...
int MediaSubsession::initiate(const std::string& own_ip) {
    ...
    do {
        ...
        if (m_client_port_num != 0 && is_multicast_address(temp_addr.s_addr)) {
            ...
            m_rtp_socket = new RtpInterface(scheduler, connection_endpoint_name(), server_port_num());
            ...
            if (protocol_is_rtp) {
                if (m_multiplex_rtcp_with_rtp)
                    ...
                    m_rtcp_socket = new RtpInterface(scheduler, connection_endpoint_name(), server_port_num() + 1);
        ...
        if (m_rtcp_socket) {
            m_rtcp = new Rtcp(parent_session().rtsp_client()->scheduler(), m_rtcp_socket, m_parent.CNAME(), this);
            if (!m_rtcp) {
    ...
    do {
        if (!strcmp(m_protocol_name, "UDP")) {
            ...
            if (!strcmp(m_codec_name, "H264")) {
                m_rtp_source = new H264VideoRTPSource(parent_session().rtsp_client()->scheduler(), m_rtp_socket,
                                                      m_rtp_payload_format, m_rtp_timestamp_frequency,
                                                      attr_val_str("sprop-parameter-sets"), parent_session().opaque());
            } else if (!strcmp(m_codec_name, "MPEG4-GENERIC")) {
                const char* fmtp_config = attr_val_str("config");
                if (!strlen(fmtp_config))
                    fmtp_config = attr_val_str("configuration");
                m_rtp_source = new MPEG4GenericRTPSource(
                        parent_session().rtsp_client()->scheduler(), m_rtp_socket, m_rtp_payload_format,
                        m_rtp_timestamp_frequency, m_medium_name, attr_val_str2lower("mode"),
                        attr_val_unsigned("sizelength"), attr_val_unsigned("indexlength"),
                        attr_val_unsigned("indexdeltalength"), fmtp_config, parent_session().opaque());
            } else {
                LOGE("RTP payload format \"%s\" unknown or not supported", m_codec_name);
                break;
            }
        }
        return 0;
