...
MediaSession::MediaSession(RtspClient* rtsp_client, void* opaque)
        : m_client(rtsp_client),
          ...
          m_control_path(NULL),
          m_max_play_start_time(0.0f),
          m_max_play_end_time(0.0f),
...
int MediaSession::initialize_with_sdp(const std::string& sdp) {
    ...
    for (;;) {
        if (parse_sdp_line(sdp_line, next_sdp_line) < 0)
            return -1;
        if (sdp_line[0] == 'm')
            break;
        ...
        if (!parse_sdp_line_s(sdp_line))
            continue;
        if (!parse_sdp_line_i(sdp_line))
            continue;
        if (!parse_sdp_line_c(sdp_line))
            continue;
        if (!parse_sdp_attr_control(sdp_line))
            continue;
        if (!parse_sdp_attr_range(sdp_line))
            continue;
        if (!parse_sdp_attr_type(sdp_line))
            continue;
        if (!parse_sdp_attr_source_filter(sdp_line))
            continue;
    }
    ...
    while (sdp_line != NULL) {
        ...
        if ((sscanf(sdp_line, "m=%s %hu RTP/AVP %u", medium_name, &subsession->m_client_port_num, &payload_format) ==
                     3 ||
             sscanf(sdp_line, "m=%s %hu/%*u RTP/AVP %u", medium_name, &subsession->m_client_port_num,
                    &payload_format) == 3) &&
            payload_format <= 127) {
            protocol_name = "RTP";
        } else if ((sscanf(sdp_line, "m=%s %hu UDP %u", medium_name, &subsession->m_client_port_num, &payload_format) ==
                            3 ||
                    sscanf(sdp_line, "m=%s %hu udp %u", medium_name, &subsession->m_client_port_num, &payload_format) ==
                            3 ||
                    sscanf(sdp_line, "m=%s %hu RAW/RAW/UDP %u", medium_name, &subsession->m_client_port_num,
                           &payload_format) == 3) &&
                   payload_format <= 127) {
            protocol_name = "UDP";
        } else {
            ...
            if (sdp_line_str != (char*)sdp_line)
                SAFE_FREE(sdp_line_str);
        ...
        for (;;) {
            ...
            if (sdp_line[0] == 'm')
                break;
            if (!subsession->parse_sdp_line_c(sdp_line))
                continue;
            if (!subsession->parse_sdp_line_b(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_rtpmap(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_rtcpmux(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_control(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_range(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_fmtp(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_source_filter(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_x_dimensions(sdp_line))
                continue;
            if (!subsession->parse_sdp_attr_framerate(sdp_line))
                continue;
        }
        if (sdp_line)
            subsession->m_saved_sdp_lines[sdp_line - start] = '\0';

        if (!subsession->m_codec_name) {
            subsession->m_codec_name =
                    lookup_payload_format(subsession->m_rtp_payload_format, subsession->m_rtp_timestamp_frequency,
                                          subsession->m_num_channels);
            if (!subsession->m_codec_name) {
                LOGE("Unknown codec name for RTP payload type", STR(sprintf_("%d", subsession->m_rtp_payload_format)));
                return -1;
            }
        }
...
int MediaSession::parse_sdp_line(const char* input_line, const char*& next_line) {
    ...
    if (input_line[0] == '\r' || input_line[0] == '\n')
        return 0;
    if (strlen(input_line) < 2 || input_line[1] != '=' || input_line[0] < 'a' || input_line[0] > 'z') {
...
int parse_range_attr(const char* sdp_line, double& start_time, double& end_time) {
    int res = sscanf(sdp_line, "a=range: npt = %lg - %lg", &start_time, &end_time);
    if (res == 2)
        return 0;
    return -1;
}
...
#ifdef XDEBUG
        LOGD("max_play_start_time: %lf, max_play_end_time: %lf", m_max_play_start_time, m_max_play_end_time);
#endif
    }
    return ret;
}
...
int parse_source_filter_attr(const char* sdp_line, char*& source_filter_name) {
    ...
    if (!source_name)
        return -1;
    if (sscanf(sdp_line, "a=source-filter: incl IN IP4 %*s %s", source_name) == 1) {
        ret = 0;
...
int MediaSession::setup_subsessions(bool stream_outgoing, bool stream_using_tcp) {
    ...
    FOR_VECTOR_ITERATOR(MediaSubsession*, m_subsessions, it) {
        if ((*it)->initiate(ap.get_address()) < 0) {
            LOGE("Failed to initiate the \"%s/%s\" subsession (cont)", (*it)->medium_name(), (*it)->codec_name());
            continue;
        }
        if ((*it)->rtcp_is_muxed()) {
            LOGI("Initiated the \"%s/%s\" subsession (client port %d)", (*it)->medium_name(), (*it)->codec_name(),
                 (*it)->client_port_num());
        } else {
            LOGI("Initiated the \"%s/%s\" subsession (client ports %d-%d)", (*it)->medium_name(), (*it)->codec_name(),
                 (*it)->client_port_num(), (*it)->client_port_num() + 1);
        }
...
int MediaSession::play_subsessions() {
    if (abs_start_time()) {
        LOGE("The stream is indexed by 'absolute' time: %s, not supported", abs_start_time());
        return -1;
    } else {
...
char* MediaSession::abs_start_time() const {
    if (m_abs_start_time)
        return m_abs_start_time;

    FOR_VECTOR_CONST_ITERATOR(MediaSubsession*, m_subsessions, it) {
        if ((*it)->_abs_start_time())
            return (*it)->_abs_start_time();
    }
    return NULL;
}
...
char* MediaSession::abs_end_time() const {
    if (m_abs_end_time)
        return m_abs_end_time;

    FOR_VECTOR_CONST_ITERATOR(MediaSubsession*, m_subsessions, it) {
        if ((*it)->_abs_end_time())
            return (*it)->_abs_end_time();
    }
    return NULL;
}
...
char* MediaSession::lookup_payload_format(unsigned char rtp_payload_type, unsigned& freq, unsigned& nchannel) {
...
unsigned MediaSession::guess_rtp_timestamp_frequency(const char* medium_name, const char* codec_name) {
    if (strcmp(codec_name, "L16") == 0)
        return 44100;
    if (strcmp(codec_name, "MPA") == 0 || strcmp(codec_name, "MPA-ROBUST") == 0 ||
        strcmp(codec_name, "X-MP3-DRAFT-00") == 0)
        return 90000;

    if (strcmp(medium_name, "video") == 0)
        return 90000;
    else if (strcmp(medium_name, "text") == 0)
        return 1000;
    return 8000;
}
