...
H264Fragmenter::H264Fragmenter(xutil::Queue<xmedia::Frame*>* queue_src, unsigned input_buffer_max,
                               unsigned max_output_packet_size)
        : m_queue_src(queue_src),
          m_input_buffer_size(input_buffer_max + 1),
          m_max_output_packet_size(max_output_packet_size),
          m_num_valid_data_bytes(1),
          m_cur_data_offset(1),
...
void H264Fragmenter::get_next_frame(unsigned char* to, unsigned max_size, after_getting_func* func, void* data) {
    m_to = to;
    ...
    if (m_num_valid_data_bytes == 1) {
        ...
        if (frame_size > m_input_buffer_size - 1) {
            LOGW("frame_size=%u, m_input_buffer_size-1=%u", frame_size, m_input_buffer_size - 1);
        ...
        after_getting_frame1(frame_size, num_truncated_bytes, presentation_time, m_duration_in_microseconds);
    } else {
        ...
        if (m_max_size < m_max_output_packet_size) {  // shouldn't happen
            LOGW("m_max_size(%u) is smaller than expected", m_max_size);
        } else {
        ...
        m_after_getting_func(m_after_getting_client_data, m_frame_size, 0, m_presentation_time,
                             m_duration_in_microseconds);
        m_duration_in_microseconds = 0;
    }
}
...
void H264Fragmenter::after_getting_frame1(unsigned frame_size, unsigned num_truncated_bytes,
    ...
    get_next_frame(m_to, m_max_size, m_after_getting_func, m_after_getting_client_data);
}
