...
Rtcp::Rtcp(TaskScheduler* scheduler, RtpInterface* interface, const char* cname, MediaSubsession* subsess)
        : m_interface(interface),
          m_subsess(subsess),
          m_type_of_event(EVENT_SDES),
          m_on_expire_task(NULL),
          m_scheduler(scheduler) {
    m_scheduler->turn_on_background_read_handling(
            m_interface->get_sockfd(), (TaskScheduler::BackgroundHandlerProc*)&Rtcp::network_read_handler, this);
...
void Rtcp::parse_rtcp_SDES(const uint8_t* pkt, size_t size) {
    ...
    while (p < pend) {
        ...
        if (b + sdes_len < bend) {
            ...
            LOGW("Unsufficient buffer to save RTCP SDES type %d:%.*s", sdes_type, sdes_len, p);
            p += sdes_len;
            continue;
        }
        ...
        switch (sdes_type) {
            ...
            case RTCP_SDES_NOTE:
                ...
                break;
            default:
                LOGW("Received unknown RTCP SDES type %d:%.*s", sdes_type, sdes_value.slen, sdes_value.ptr);
                break;
        }

#ifdef XDEBUG
        LOGD("SDES type %d:%.*s", sdes_type, sdes_value.slen, sdes_value.ptr);
#endif
...
void Rtcp::parse_rtcp_BYE(const uint8_t* pkt, size_t size) {
    ...
    LOGD("Received RTCP BYE, reasion: %.*s", reason.slen, reason.ptr);
...
void Rtcp::on_expire(void* client_data) {
    ...
    if (rtcp->m_type_of_event == EVENT_SDES) {
        ...
        LOGE("Don't support to send this type of event(%d)", rtcp->m_type_of_event);
        return;
    }
...
void Rtcp::on_expire1() {
    ...
    m_on_expire_task = m_scheduler->schedule_delayed_task(u_seconds_to_delay, on_expire, this);
}
...
void Rtcp::send_sdes() {
    ...
    uint8_t *p = (uint8_t*)(m_peer_sdes_buf + sizeof(RtcpCommon)), *psave = p;
    const char* cname = m_subsess->parent_session().CNAME();
    int len = strlen(cname);
    put_byte(p, RTCP_SDES_CNAME);
    put_byte(p, len);
    strncpy((char*)p, cname, len);
    p += len;
