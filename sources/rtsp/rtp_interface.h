...
public:
    ...
    virtual int write(const uint8_t* buf, int size, struct sockaddr_in* remote = NULL);
...
private:
    int send_rtp_or_rtcp_packet_over_tcp(uint8_t* packet, unsigned packet_size, int socket_num,
                                         unsigned char stream_channel_id);
    int send_data_over_tcp(int socket_num, uint8_t* data, unsigned data_size);
