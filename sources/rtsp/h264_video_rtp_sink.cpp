...
H264VideoRTPSink::H264VideoRTPSink(TaskScheduler* scheduler, RtpInterface* interface, unsigned char rtp_payload_format,
                                   uint8_t const* sps, unsigned sps_size, uint8_t const* pps, unsigned pps_size)
        : MultiFramedRTPSink(scheduler, interface, rtp_payload_format, 90000, "H264"),
          m_our_fragmenter(NULL),
          m_fmtp_sdp_line(NULL) {
    if (sps) {
        m_sps_size = sps_size;
...
H264VideoRTPSink::~H264VideoRTPSink() {
    ...
    SAFE_DELETE_ARRAY(m_sps);
    SAFE_DELETE_ARRAY(m_pps);
...
char const* H264VideoRTPSink::aux_sdp_line() {
    ...
    SAFE_FREE(m_fmtp_sdp_line);
    m_fmtp_sdp_line =
            strdup_(STR(sprintf_("a=fmtp:%d packetization-mode=1;profile-level-id=%06X;sprop-parameter-sets=%s,%s" CRLF,
...
bool H264VideoRTPSink::continue_playing() {
    if (!m_our_fragmenter) {
        m_our_fragmenter =
                new H264Fragmenter(m_queue_src, OutPacketBuffer::max_size, our_max_packet_size() - rtp_header_size);
    }
...
void H264VideoRTPSink::do_special_frame_handling(unsigned fragmentation_offset, unsigned char* frame_start,
    ...
    if (m_our_fragmenter) {
        if (m_our_fragmenter->last_fragment_completed_nal_unit() && m_our_fragmenter->picture_end_marker()) {
            set_marker_bit();
        }
    }
