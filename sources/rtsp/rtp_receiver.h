...
enum { kStartNumberOfFrames = 6, kMaxNumberOfFrames = 100 };
...
class Packet {
public:
    Packet(const uint8_t* ptr, const uint32_t size, uint16_t seq_num, uint32_t timestamp, bool marker_bit,
           FrameType ft = kFrameUnknown, bool current_packet_begins_frame = true, bool first_packet = false);
    ~Packet();
    ...
    const uint16_t seq_num() const {
        return m_seq_num;
    }
    const uint32_t timestamp() const {
        return m_timestamp;
    }
    const bool marker_bit() const {
        return m_marker_bit;
    }
    ...
    void set_frame_type(const FrameType ft) {
        m_frame_type = ft;
    }
    const bool& current_packet_begins_frame() const {
        return m_current_packet_begins_frame;
    }
    const bool& first_packet() const {
        return m_first_packet;
    }
...
public:
    ...
    const FrameBufferStateEnum get_state() {
        return m_state;
    }
...
public:
    ...
    const bool in_initial_state() const {
        return m_in_initial_state;
    }
...
public:
    ...
    FrameBuffer* frame_at(Iterator it) {
        return MAP_VAL(it);
    }
...
public:
    ...
    FrameBufferEnum insert_packet(const Packet* pkt, CompleteFrameProc cb, void* client_data);
...
inline bool is_newer_seq_num(uint16_t seq_num, uint16_t prev_seq_num) {
    return seq_num != prev_seq_num && (uint16_t)(seq_num - prev_seq_num) < 0x8000;
}
...
inline uint16_t latest_seq_num(uint16_t seq_num1, uint16_t seq_num2) {
    return is_newer_seq_num(seq_num1, seq_num2) ? seq_num1 : seq_num2;
}
