...
#define SOCKET_EXCEPTION (1 << 3)
    ...
    bool quit() const {
        return m_watch_variable ? *m_watch_variable : false;
    }

    int single_step(unsigned max_delay_time = 10000);

    void turn_on_background_read_handling(int socket_num, BackgroundHandlerProc* handler_proc, void* client_data) {
    ...
    void set_background_handling(int socket_num, int condition_set, BackgroundHandlerProc* handler_proc,
                                 void* client_data);
    void disable_background_handling(int socket_num) {
        set_background_handling(socket_num, 0, NULL, NULL);
    }

    TaskToken schedule_delayed_task(int64_t microseconds, TaskFunc* proc, void* client_data);
...
public:
    ...
    void assign_handler(int socket_num, int condition_set, TaskScheduler::BackgroundHandlerProc* handler_proc,
                        void* client_data);
