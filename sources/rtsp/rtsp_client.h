...
enum TransportMode { RtpUdp = 1, RtpTcp, RawUdp };
...
public:
    ...
    int request_setup(MediaSubsession* subsession, bool stream_outgoing = false, bool stream_using_tcp = false,
                      bool force_multicast_on_unspecified = false);
    int request_play(MediaSession* session, double start = 0.0f, double end = -1.0f, float scale = 1.0f);

    void construct_subsession_url(MediaSubsession const* subsession, const char*& prefix, const char*& separator,
                                  const char*& suffix);
...
private:
    ...
    static bool parse_response_code(char* line, unsigned& response_code, char*& response_string);
    static bool check_for_header(char* line, char const* header_name, unsigned header_name_length, char*& header_parm);

    int parse_transport_parms(const char* parms_str, char*& server_address_str, PortNumBits& server_port_num);

    char* create_blocksize_string(bool stream_using_tcp);

    std::string generate_cmd_url(const char* base_url, MediaSession* session = NULL,
                                 MediaSubsession* subsession = NULL);

    void schedule_liveness_command();
    static void send_liveness_command(void* client_data);

    static bool rtsp_option_is_supported(const char* command_name, const char* public_parm_str);
...
private:
    ...
    TaskFunc *m_continue_after_options, *m_continue_after_get_parameter;
