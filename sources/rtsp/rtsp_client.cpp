...
int RtspClient::open(const std::string& url, AddressPort& ap) {
    if (m_stat != StateInit)
        return -1;
    ...
    LOGI("Connected to rtsp server: %s successfully", STR(_url.srvap.to_string()));
...
int RtspClient::request_options(TaskFunc* proc) {
    ...
    if (m_stat == StatePlaying && m_tcp_stream_id_count != 0) {
    ...
    m_server_supports_get_parameter = rtsp_option_is_supported("GET_PARAMETER", ri.public_parm_str);
...
int RtspClient::send_request(const char* cmd_url, const std::string& request, const std::string& content) {
    ++m_cseq;
    string str(sprintf_("%s %s RTSP/1.0" CRLF "CSeq: %d" CRLF "User-Agent: %s" CRLF "%s" CRLF "%s", STR(request),
                        cmd_url, m_cseq, STR(m_user_agent_str), STR(field2string()), STR(content)));
...
void RtspClient::handle_alternative_request_byte1(uint8_t request_byte) {
    ...
    if (ret < 0) {
        ...
        if (m_requests.pop(str) == 0) {
            if (!strcmp(STR(str), "OPTIONS") && m_continue_after_options) {
                m_continue_after_options(this);
            } else if (!strcmp(STR(str), "GET_PARAMETER") && m_continue_after_get_parameter) {
...
int RtspClient::recv_response(ResponseInfo* pri, uint8_t request_byte) {
    ...
    do {
...
#ifdef XDEBUG_RTSP_MESSAGE
        LOGD("response_code: %u, response_str:%s, session_parm_str:%s, transport_parm_str:%s, scale_parm_str:%s, "
             "range_parm_str:%s, rtp_info_parm_str:%s, public_parm_str:%s, content_base_parm_str:%s, "
             "content_type_parm_str:%s",
             pri->response_code, pri->response_str, pri->session_parm_str, pri->transport_parm_str, pri->scale_parm_str,
             pri->range_parm_str, pri->rtp_info_parm_str, pri->public_parm_str, pri->content_base_parm_str,
             pri->content_type_parm_str);
#endif

        unsigned body_offset = next_line_start == NULL ? m_rrb.nread : next_line_start - header_data_copy;
        if (content_length) {
            ...
            if (content_length > pri->num_body_bytes) {
                ...
                if (num_extra_bytes_needed > remaining_buffer_size) {
                    LOGW("Response buffer size (%d) is too small for \"Content-Length:\" %d", RTSP_MSG_BUFSIZ,
                         content_length);
                    ret = -1;
                    break;
                }
        ...
        int num_extra_bytes_after_response = m_rrb.nread - (body_offset + content_length);
        if (num_extra_bytes_after_response != 0) {
            memmove(m_rrb.buf, m_rrb.buf + body_offset + content_length, num_extra_bytes_after_response);
...
bool RtspClient::parse_response_code(char* line, unsigned& response_code, char*& response_string) {
...
bool RtspClient::check_for_header(char* line, char const* header_name, unsigned header_name_length,
                                  char*& header_parm) {
    if (strncasecmp(line, header_name, header_name_length))
        return false;

    unsigned parm_index = header_name_length;
    while (line[parm_index] != '\0' && (line[parm_index] == ' ' || line[parm_index] == '\t'))
        ++parm_index;
...
string RtspClient::generate_cmd_url(const char* base_url, MediaSession* session, MediaSubsession* subsession) {
    if (subsession) {
...
int RtspClient::request_play() {
    if (m_sess->play_subsessions() == 0) {
        if (m_duration > 0) {
            ...
            m_stream_timer_task =
                    m_scheduler->schedule_delayed_task(u_secs_to_delay, (TaskFunc*)stream_timer_handler, this);
        }
        LOGI("Started playing session (for up to %.3lf seconds) ...", m_duration);
        return 0;
    }
    return (-1);
}
...
int RtspClient::request_setup(MediaSubsession* subsession, bool stream_outgoing, bool stream_using_tcp,
                              bool force_multicast_on_unspecified) {
    ...
    unsigned transport_size =
            strlen(transport_fmt) + strlen(transport_type_str) + strlen(mode_str) + strlen(port_type_str) + 2 * 5;
    char* transport_str = (char*)malloc(transport_size);
    sprintf(transport_str, transport_fmt, transport_type_str, mode_str, port_type_str, rtp_number, rtcp_number);
    ...
    SAFE_FREE(transport_str);
    SAFE_FREE(session_str);
    SAFE_FREE(blocksize_str);
    ...
    if (send_request(STR(cmd_url), "SETUP") > 0) {
        ...
        if (!recv_response(&ri) && ri.response_code == 200) {
            ...
            if (session_id) {
                do {
                    if (!ri.session_parm_str || sscanf(ri.session_parm_str, "%[^;]", session_id) != 1) {
                        LOGE("Missing or bad \"Session:\" header");
                        break;
                    }
                    subsession->set_session_id(session_id);
                    SAFE_FREE(m_last_session_id);
                    m_last_session_id = strdup(session_id);
            ...
            if (parse_transport_parms(ri.transport_parm_str, server_address_str, server_port_num) < 0) {
...
int RtspClient::parse_transport_parms(const char* parms_str, char*& server_address_str, PortNumBits& server_port_num) {
    server_address_str = NULL;
    server_port_num = 0;
    if (!parms_str || !strlen(parms_str))
        return -1;
...
static char* create_range_string(double start, double end, const char* abs_start_time, const char* abs_end_time) {
    char buf[100];
...
int RtspClient::request_play(MediaSession* session, double start, double end, float scale) {
    if (!m_last_session_id) {
        LOGE("No RTSP session is currently in progress");
        return -1;
    }
...
void RtspClient::construct_subsession_url(MediaSubsession const* subsession, const char*& prefix,
...
#ifdef XDEBUG
    LOGD("Will send_liveness_command() in %.2f secs", ((double)u_seconds_to_delay) / MILLION);
#endif

    if (m_liveness_command_task)
        m_scheduler->unschedule_delayed_task(m_liveness_command_task);
    m_liveness_command_task = m_scheduler->schedule_delayed_task(u_seconds_to_delay, send_liveness_command, this);
}
...
bool RtspClient::rtsp_option_is_supported(const char* command_name, const char* public_parm_str) {
...
std::string RtspUrl::to_string() const {
    return sprintf_("rtsp://%s%s/%s", username.empty() ? "" : STR(username + ":" + passwd + "@"),
                    STR(srvap.to_string()), STR(stream_name));
}
...
Rtsp::Rtsp() : m_stat(StateInit), m_cseq(0) {
}
...
int Rtsp::parse_url(const string surl, RtspUrl& rtsp_url) {
    ...
    int i;
    for (i = 0; from[i] && from[i] != ':' && from[i] != '/'; ++i)
        tmp[i] = from[i];
    tmp[i] = '\0';
    rtsp_url.srvap.set_address(tmp);

    if (from[i] == ':') {
        for (p = from + i + 1, i = 0; *p && *p != '/'; ++i, ++p)
            tmp[i] = *p;
        tmp[i] = '\0';
