...
public:
    MultiFramedRTPSink(TaskScheduler* scheduler, RtpInterface* interface, uint8_t rtp_payload_type,
    ...
    uint8_t rtp_payload_type() const {
        return m_rtp_payload_type;
    }
    unsigned rtp_timestamp_frequency() const {
        return m_rtp_timestamp_frequency;
    }
    void set_rtp_timestamp_frequency(unsigned freq) {
        m_rtp_timestamp_frequency = freq;
    }
    const char* rtp_payload_format_name() const {
        return m_rtp_payload_format_name;
    }
    ...
    uint16_t current_seq_num() const {
        return m_seq_num;
    }
    ...
    virtual RtpInterface* rtp_interface() const {
        return m_interface;
    }
...
protected:
    ...
    virtual void do_special_frame_handling(unsigned fragmentation_offset, unsigned char* frame_start,
    ...
    bool is_first_packet() const {
        return m_is_first_packet;
    }
    bool is_first_frame_in_packet() const {
        return m_num_frames_used_so_far == 0;
    }
    unsigned cur_fragmentation_offset() const {
        return m_cur_fragmentation_offset;
    }
    void set_marker_bit();
    ...
    void set_special_header_bytes(unsigned char const* bytes, unsigned num_bytes, unsigned byte_position = 0);
    void set_frame_specific_header_word(unsigned word, unsigned word_position = 0);
    void set_frame_specific_header_bytes(unsigned char const* bytes, unsigned num_bytes, unsigned byte_position = 0);
    void set_frame_padding(unsigned num_padding_bytes);
    unsigned num_frames_used_so_far() const {
        return m_num_frames_used_so_far;
    }
    unsigned our_max_packet_size() const {
        return m_our_max_packet_size;
    }
...
private:
    ...
    static void after_getting_frame(void* client_data, unsigned num_bytes_read, unsigned num_truncated_bytes,
                                    struct timeval presentation_time, unsigned duration_in_microseconds);
    void after_getting_frame1(unsigned frame_size, unsigned num_truncated_bytes, struct timeval presentation_time,
