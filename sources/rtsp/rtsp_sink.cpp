...
public:
    ...
    MultiFramedRTPSink* rtp_sink() const {
        return m_rtp_sink;
    }
    Rtcp* rtcp() const {
        return m_rtcp;
    }
    char const* sdp_lines() const {
        return m_sdp_lines;
    }
...
SubstreamDescriptor::SubstreamDescriptor(MultiFramedRTPSink* rtp_sink, Rtcp* rtcp, unsigned track_id)
    ...
    m_sdp_lines = strdup_(STR(sprintf_("m=%s 0 RTP/AVP %u" CRLF "%s"
                                       "%s"
                                       "a=control:trackID=%u" CRLF,
                                       m_rtp_sink->sdp_media_type(), m_rtp_sink->rtp_payload_type(), rtpmap_line,
                                       aux_sdp_line, track_id)));
...
RtspSink::RtspSink(const std::string& flvpath)
        : MediaSink(flvpath),
          ...
          m_last_track_id(0),
          m_video(new MediaAggregation),
          m_audio(new MediaAggregation),
          m_send_error(false),
          m_start_sink(false),
...
int RtspSink::send_video(int32_t timestamp, byte* dat, uint32_t length, uint32_t composition_time) {
    Frame* f = new Frame;
    ...
    if (!m_video->sink) {
        ...
        if (!vparser.process(dat, length) && vparser.is_key_frame() && vparser.get_sps_length() &&
            vparser.get_pps_length()) {
            ...
            m_video->sink = new H264VideoRTPSink(scheduler, m_video->rtp_socket, 96, vparser.get_sps(),
                                                 vparser.get_sps_length(), vparser.get_pps(), vparser.get_pps_length());
    ...
    if (!m_start_sink && (timestamp - m_first_key_frame_timestamp) > RTSP_SINK_BUFFERING_TIME_AFTER_KEY_FRAME &&
        m_video->sink) {
        m_start_sink = true;
    ...
    if (!m_audio->sink) {
        ...
        if (!aparser.process(dat, length)) {
            ...
            m_audio->sink = new MPEG4GenericRTPSink(scheduler, m_audio->rtp_socket, 97,
                                                    atoi(samplerate_idx_to_str(sample_rate_idx)), "audio", "AAC-hbr",
...
int RtspSink::set_destination_and_play() {
    ...
    std::string sdp = sprintf_("v=0" CRLF "o=- %u %u IN IP4 %s" CRLF "s=flvpusher" CRLF "i=flvpusher" CRLF
                               "c=IN IP4 %s" CRLF "t=0 0" CRLF "a=x-qt-text-nam:" CRLF "a=x-qt-text-inf:" CRLF
                               "a=x-qt-text-cmt:source application:flvpusher" CRLF "a=x-qt-text-aut:" CRLF
                               "a=x-qt-text-cpy:" CRLF,
                               sdp_session_id, sdp_version, m_our_ap.get_address(), uri_parser->host);
    FOR_VECTOR_ITERATOR(SubstreamDescriptor*, m_substream_descriptors, it) {
        sdp += (*it)->sdp_lines();
    }
    ...
    RtpInterface::set_server_request_alternative_byte_handler(m_client->get_sockfd(),
...
RtspSink::MediaAggregation::MediaAggregation() : sink(NULL), rtcp(NULL), rtp_socket(NULL), rtcp_socket(NULL) {
}
