...
MPEG4GenericRTPSource::MPEG4GenericRTPSource(TaskScheduler* scheduler, RtpInterface* interface,
                                             unsigned char rtp_payload_format, unsigned rtp_timestamp_frequency,
                                             const char* medium_name, const char* mode, unsigned size_length,
                                             unsigned index_length, unsigned index_delta_length,
                                             const char* fmtp_config, void* opaque)
        : MultiFramedRTPSource(scheduler, interface, rtp_payload_format, rtp_timestamp_frequency, opaque),
          m_size_length(size_length),
          m_index_length(index_length),
          m_index_delta_length(index_delta_length),
          m_num_au_headers(0),
          m_next_au_header(0),
          m_au_headers(NULL) {
    unsigned mime_type_length = strlen(medium_name) + strlen("/MPEG4-GENERIC") + 1;
...
bool MPEG4GenericRTPSource::process_special_header(uint8_t* payload, unsigned payload_len, bool marker_bit,
                                                   unsigned& result_special_header_size) {
    ...
    if (m_size_length > 0) {
        ...
        if (bits_avail >= 0 && (m_size_length + m_index_delta_length) > 0)
...
const unsigned MPEG4GenericRTPSource::next_enclosed_frame_size(unsigned data_size) {
    ...
    if (m_next_au_header >= num_au_headers) {
        LOGE("next_enclosed_frame_size(%u): data error(%p,%u,%u)!", data_size, au_header, m_next_au_header,
             num_au_headers);
        return data_size;
    }
...
static bool get_nibble(const char*& config_str, uint8_t& result_nibble) {
    char c = config_str[0];
    if (c == '\0')
        return false;
...
static bool get_byte(const char*& config_str, uint8_t& result_byte) {
    ...
    if (!get_nibble(config_str, first_nibble))
        return false;
...
static uint8_t* parse_general_config_str(const char* config_str, unsigned& config_size) {
    uint8_t* config = NULL;
    ...
    do {
        ...
        for (i = 0; i < config_size; ++i)
            if (!get_byte(config_str, config[i]))
                break;
        if (i != config_size)
            break;
...
int MPEG4GenericRTPSource::on_complete_frame1(FrameBuffer* frame) {
    uint8_t* buf = (uint8_t*)m_mem_holder.alloc(AAC_ADTS_HEADER_SIZE + frame->size_bytes());
    unsigned config_size;
    uint8_t* asc_buf = parse_general_config_str(m_fmtp_config, config_size);
    if (generate_adts_header(asc_buf, frame->size_bytes(), buf) < 0) {
        LOGE("generate_adts_header failed (0x%02x 0x%02x)", asc_buf[0], asc_buf[1]);
        SAFE_FREE(asc_buf);
        return -1;
    }
...
#ifdef XDEBUG
    LOGD("AUDIO frame: %05u~%05u, timestamp=%u, nwritten=%d", frame->get_low_seq_num(), frame->get_high_seq_num(),
         frame->timestamp(), nwritten);
#endif

    if (m_opaque) {
        ((MediaPusher*)m_opaque)
                ->on_frame(
