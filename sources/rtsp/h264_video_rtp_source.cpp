...
SPropRecord* parse_s_prop_parm_str(const char* parm_str, unsigned& num_s_prop_records) {
    ...
    for (unsigned i = 0; i < num_s_prop_records; ++i) {
        result_array[i].s_prop_bytes() = base64_decode(s, strlen(s), result_array[i].s_prop_length());
        s += strlen(s) + 1;
    }
...
H264VideoRTPSource::H264VideoRTPSource(TaskScheduler* scheduler, RtpInterface* interface,
    ...
    SPropRecord* s_prop_records = parse_s_prop_parm_str(s_prop_parm_str, num_s_prop_records);
    for (unsigned i = 0; i < num_s_prop_records; ++i) {
...
bool H264VideoRTPSource::process_special_header(uint8_t* payload, unsigned payload_len, bool marker_bit,
...
int H264VideoRTPSource::on_complete_frame1(FrameBuffer* frame) {
    ...
    if (frame->frame_type() == kFrameKey && m_sps_size && m_pps_size) {
        ...
        nwritten += 4;
        memcpy(buf + nwritten, m_sps, m_sps_size);
        nwritten += m_sps_size;
        memcpy(buf + nwritten, nalu_startcode, 4);
        nwritten += 4;
        memcpy(buf + nwritten, m_pps, m_pps_size);
        nwritten += m_pps_size;
    }
...
#ifdef XDEBUG
    LOGD("VIDEO frame: %05u~%05u, timestamp=%u, bytes_max=%u, nwritten=%d", frame->get_low_seq_num(),
         frame->get_high_seq_num(), frame->timestamp(), bytes_max, nwritten);
#endif
