...
MultiFramedRTPSource::MultiFramedRTPSource(TaskScheduler* scheduler, RtpInterface* interface,
...
int MultiFramedRTPSource::start_receiving() {
    ...
    m_scheduler->turn_on_background_read_handling(
            m_interface->get_sockfd(),
...
void MultiFramedRTPSource::network_read_handler1(int mask) {
    ...
    if (process_special_header((uint8_t*)payload, payload_len, header.marker_bit, special_header_size)) {
    ...
    switch (codec_id()) {
        case CODEC_ID_H264:
            if (m_current_packet_begins_frame) {
                ...
                if (nalu_type == 0x07 || nalu_type == 0x08 || nalu_type == 0x05 || nalu_type == 0x06) {
                    ft = kFrameKey;
                    if (header.marker_bit) {
                         ...
                         * SEI, timestamp:0, m:1
                         * IDR, timestamp:0, m:0 ... */
                        if (nalu_type == 0x07 || (nalu_type != 0x07 && header.timestamp == m_last_received_timestamp))
    ...
    do {
        ...
        Packet* pkt = new Packet((uint8_t*)(payload + payload_offset), cur_frame_size, header.sequence_number,
                                 header.timestamp, header.marker_bit, ft, m_current_packet_begins_frame,
                                 is_first_pkt_in_frame);
        m_receiver.insert_packet(pkt, (Receiver::CompleteFrameProc*)&MultiFramedRTPSource::on_complete_frame, this);
...
int MultiFramedRTPSource::on_complete_frame(MultiFramedRTPSource* source, FrameBuffer* frame) {
