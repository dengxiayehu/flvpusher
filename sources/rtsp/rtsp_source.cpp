...
RtspSource::RtspSource(const std::string& input, MediaSink*& sink) : MediaPusher(input, sink) {
...
int RtspSource::prepare() {
    ...
    if (m_client->request_options((TaskFunc*)RtspClient::continue_after_options) < 0) {
        LOGE("Failed to send OPTIONS command");
        return -1;
    }

    string sdp;
    if (m_client->request_describe(sdp, (TaskFunc*)RtspClient::continue_after_describe) < 0) {
...
int RtspSource::on_frame(const int32_t ts, const byte* dat, const uint32_t dat_len, int is_video) {
