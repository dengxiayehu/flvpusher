...
SocketDescriptor::SocketDescriptor(TaskScheduler* scheduler, int socket_num)
        : m_scheduler(scheduler),
          m_our_socket_num(socket_num),
          m_server_request_alternative_byte_handler(NULL),
          m_server_request_alternative_byte_handler_client_data(NULL),
          m_read_error_occurred(false),
          m_delete_myself_next(false),
          m_are_in_read_handler_loop(false),
...
void SocketDescriptor::register_interface(unsigned char stream_channel_id, void* interface) {
    ...
    if (is_first_registration) {
        m_scheduler->turn_on_background_read_handling(m_our_socket_num,
                                                      (TaskScheduler::BackgroundHandlerProc*)&tcp_read_handler, this);
    }
}
...
void SocketDescriptor::tcp_read_handler(SocketDescriptor* socket_descriptor, int mask) {
    ...
    while (!socket_descriptor->m_delete_myself_next && socket_descriptor->tcp_read_handler1(mask) && --count > 0) {
    }
...
bool SocketDescriptor::tcp_read_handler1(int mask) {
    ...
    if (m_tcp_reading_state != AWAITING_PACKET_DATA) {
        ...
        if (!result) {
            ...
            LOGE("socket_num (%d) recv failed: %s", m_our_socket_num, ERRNOMSG);
    ...
    switch (m_tcp_reading_state) {
        case AWAITING_DOLLAR:
            if (c == '$') {
                ...
                if (m_server_request_alternative_byte_handler && c != 0xFF && c != 0xFE) {
        ...
        case AWAITING_STREAM_CHANNEL_ID:
            if (lookup_interface(c) != NULL) {
                ...
                // This wasn't a stream channel id that we expected.  We're (somehow) in a strange state.  Try to
                // recover:
...
void SocketDescriptor::deregister_interface(unsigned char stream_channel_id) {
    ...
    if (m_sub_channel_map.empty() || stream_channel_id == 0xFF) {
...
RtpInterface::RtpInterface(TaskScheduler* scheduler, const AddressPort& remote) : Udp(remote), m_scheduler(scheduler) {
}
...
void RtpInterface::set_stream_socket(int sockfd, unsigned char stream_channel_id) {
    ...
    FOR_VECTOR_CONST_ITERATOR(TcpStreamRecord*, m_tcp_stream_record, it) {
        if ((*it)->m_stream_socket_num == sockfd && (*it)->m_stream_channel_id == stream_channel_id) {
            LOGW("sockfd(%d) with stream_channel_id(%d) already registered", sockfd, stream_channel_id);
            return;
        }
    }
...
void RtpInterface::remove_stream_socket(int sock_num, unsigned char stream_channel_id) {
    for (vector<TcpStreamRecord*>::iterator it = m_tcp_stream_record.begin(); it != m_tcp_stream_record.end();) {
...
int RtpInterface::write(const uint8_t* buf, int size, struct sockaddr_in* remote) {
    ...
    int ret = -1;
    FOR_VECTOR_CONST_ITERATOR(TcpStreamRecord*, m_tcp_stream_record, it) {
        if (send_rtp_or_rtcp_packet_over_tcp((uint8_t*)buf, size, (*it)->m_stream_socket_num,
                                             (*it)->m_stream_channel_id) < 0) {
            return -1;
        } else
            ret = 0;
    }
...
int RtpInterface::send_rtp_or_rtcp_packet_over_tcp(uint8_t* packet, unsigned packet_size, int socket_num,
                                                   unsigned char stream_channel_id) {
#ifdef XDEBUG
    LOGD("%d bytes over channel %d (socket %d)", packet_size, stream_channel_id, socket_num);
#endif
    ...
    if (!send_data_over_tcp(socket_num, framing_header, 4) && !send_data_over_tcp(socket_num, packet, packet_size))
        return 0;

    return -1;
}

int RtpInterface::send_data_over_tcp(int socket_num, uint8_t* data, unsigned data_size) {
