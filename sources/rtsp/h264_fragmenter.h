...
typedef void(after_getting_func)(void* client_data, unsigned frame_size, unsigned num_truncated_bytes,
                                 struct timeval presentation_time, unsigned duration_in_microseconds);

class H264Fragmenter {
public:
    H264Fragmenter(xutil::Queue<xmedia::Frame*>* queue_src, unsigned input_buffer_max, unsigned max_output_packet_size);
    ~H264Fragmenter();

    bool last_fragment_completed_nal_unit() const {
        return m_last_fragment_completed_nal_unit;
    }
