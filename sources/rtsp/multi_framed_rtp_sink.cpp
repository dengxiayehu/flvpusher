...
MultiFramedRTPSink::MultiFramedRTPSink(TaskScheduler* scheduler, RtpInterface* interface, uint8_t rtp_payload_type,
                                       uint32_t rtp_timestamp_frequency, const char* rtp_payload_format_name,
        ...
        : m_scheduler(scheduler),
          ...
          m_on_send_error_func(NULL),
          m_on_send_error_data(NULL),
          m_next_task(NULL),
...
void MultiFramedRTPSink::set_packet_sizes(unsigned preferred_packet_size, unsigned max_packet_size) {
    if (preferred_packet_size > max_packet_size || preferred_packet_size == 0) {
        LOGW("preferred_packet_size=%u, max_packet_size=%u (ignored)", preferred_packet_size, max_packet_size);
        return;
    }
...
char* MultiFramedRTPSink::rtpmap_line() const {
    if (rtp_payload_type() >= 96) {
        ...
        unsigned rtpmap_fmt_size =
                strlen(rtpmap_fmt) + 3 + strlen(rtp_payload_format_name()) + 20 + encoding_params_part.length();
        char* rtpmap_line = (char*)malloc(rtpmap_fmt_size);
        snprintf(rtpmap_line, rtpmap_fmt_size, rtpmap_fmt, rtp_payload_type(), rtp_payload_format_name(),
...
#ifdef XDEBUG
    LOGD("m_timestamp_base: 0x%08x, tv: %lu.%06ld\n\t=> RTP timestamp: 0x%08x", m_timestamp_base, tv.tv_sec, tv.tv_usec,
         rtp_timestamp);
#endif
...
void MultiFramedRTPSink::do_special_frame_handling(unsigned fragmentation_offset, unsigned char* frame_start,
                                                   unsigned num_bytes_in_frame, struct timeval frame_presentation_time,
    ...
    if (m_out_buf->have_overflow_data()) {
        ...
        if (!strcmp(sdp_media_type(), "video")) {
            ...
            if (m_queue_src->pop(f) == 0) {
                ...
                after_getting_frame(this, f->get_data_length() - 7, 0, presentation_time,
                                    (f->get_dts() - m_last_audio_timestamp) * 1000);
...
void MultiFramedRTPSink::send_packet_if_necessary() {
    ...
    if (m_out_buf->have_overflow_data() && m_out_buf->total_bytes_available() > m_out_buf->total_buffer_size() / 2) {
        // Efficiency hack: Reset the packet start pointer to just in front of
        // the overflow data (allowing for the RTP header and special headers),
        // so that we probably don't have to "memmove" the overflow data
        // into place when building the next packet:
        unsigned new_packet_start =
                m_out_buf->cur_packet_size() - (rtp_header_size + m_special_header_size + frame_special_header_size());
...
void MultiFramedRTPSink::after_getting_frame(void* client_data, unsigned num_bytes_read, unsigned num_truncated_bytes,
                                             struct timeval presentation_time, unsigned duration_in_microseconds) {
    MultiFramedRTPSink* sink = (MultiFramedRTPSink*)client_data;
    sink->after_getting_frame1(num_bytes_read, num_truncated_bytes, presentation_time, duration_in_microseconds);
}
...
void MultiFramedRTPSink::after_getting_frame1(unsigned frame_size, unsigned num_truncated_bytes,
    ...
    if (m_num_frames_used_so_far > 0) {
        if ((m_previous_frame_ended_fragmentation && !allow_other_frames_after_last_fragment()) ||
            ...
            m_out_buf->set_overflow_data(m_out_buf->cur_packet_size(), frame_size, presentation_time,
    ...
    if (num_frame_bytes_to_use > 0) {
        ...
        if (m_out_buf->would_overflow(frame_size)) {
            ...
            m_out_buf->set_overflow_data(m_out_buf->cur_packet_size() + num_frame_bytes_to_use, overflow_bytes,
    ...
    if (num_frame_bytes_to_use == 0 && frame_size > 0) {
        ...
        do_special_frame_handling(cur_fragmentation_offset, frame_start, num_frame_bytes_to_use, presentation_time,
                                  overflow_bytes);
        ...
        if (m_out_buf->is_preferred_size() || m_out_buf->would_overflow(num_frame_bytes_to_use) ||
