...
public:
    ...
    static char* lookup_payload_format(unsigned char rtp_payload_type, unsigned& freq, unsigned& nchannel);
    static unsigned guess_rtp_timestamp_frequency(const char* medium_name, const char* codec_name);

    double& play_start_time() {
        return m_max_play_start_time;
    }
    double& play_end_time() {
        return m_max_play_end_time;
    }
    const char* connection_endpoint_name() const {
