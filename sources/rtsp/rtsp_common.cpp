...
DelayQueueEntry::DelayQueueEntry(timeval tv) : m_delta_time_remaining(tv) {
    m_next = m_prev = this;
...
DelayQueue::DelayQueue() : DelayQueueEntry((timeval){INT_MAX, MILLION - 1}), m_last_sync_time(DELAY_ZERO) {
}
...
void DelayQueue::add_entry(DelayQueueEntry* new_entry) {
    ...
    while (new_entry->m_delta_time_remaining >= cur->m_delta_time_remaining) {
        new_entry->m_delta_time_remaining -= cur->m_delta_time_remaining;
        cur = cur->m_next;
    }

    cur->m_delta_time_remaining -= new_entry->m_delta_time_remaining;
...
int TaskScheduler::single_step(unsigned max_delay_time) {
    ...
    if (max_delay_time > 0 &&
        (tv.tv_sec > (long)max_delay_time / MILLION ||
         (tv.tv_sec == (long)max_delay_time / MILLION && tv.tv_usec > (long)max_delay_time % MILLION))) {
    ...
    while (it != m_handlers->end()) {
        ...
        if ((result_condition_set & (*it)->condition_set) != 0 && (*it)->handler_proc != NULL) {
...
void TaskScheduler::set_background_handling(int socket_num, int condition_set, BackgroundHandlerProc* handler_proc,
                                            void* client_data) {
    if (socket_num < 0)
        return;
    ...
    if (!condition_set) {
        ...
        if (condition_set & SOCKET_READABLE)
            FD_SET((unsigned)socket_num, &m_read_set);
        if (condition_set & SOCKET_WRITABLE)
            FD_SET((unsigned)socket_num, &m_write_set);
        if (condition_set & SOCKET_EXCEPTION)
            FD_SET((unsigned)socket_num, &m_exception_set);
    }
}

TaskToken TaskScheduler::schedule_delayed_task(int64_t microseconds, TaskFunc* proc, void* client_data) {
    if (microseconds < 0)
        microseconds = 0;
...
void HandlerSet::assign_handler(int socket_num, int condition_set, TaskScheduler::BackgroundHandlerProc* handler_proc,
                                void* client_data) {
