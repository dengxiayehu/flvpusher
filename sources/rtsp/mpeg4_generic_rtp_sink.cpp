...
MPEG4GenericRTPSink::MPEG4GenericRTPSink(TaskScheduler* scheduler, RtpInterface* interface,
                                         unsigned char rtp_payload_format, uint32_t rtp_timestamp_frequency,
                                         char const* sdp_media_type_string, char const* mpeg4_mode,
                                         char const* config_string, unsigned num_channels)
        : MultiFramedRTPSink(scheduler, interface, rtp_payload_format, rtp_timestamp_frequency, "MPEG4-GENERIC",
                             num_channels),
    ...
    if (strcasecmp(mpeg4_mode, "aac-hbr") != 0) {
        LOGE("Unknown \"mpeg4_mode\" parameter: %s", mpeg4_mode);
    }

    m_fmtp_sdp_line = strdup_(STR(sprintf_(
            "a=fmtp:%d "
            "streamtype=5;profile-level-id=1;mode=%s;sizelength=13;indexlength=3;indexdeltalength=3;config=%s" CRLF,
...
void MPEG4GenericRTPSink::do_special_frame_handling(unsigned fragmentation_offset, unsigned char* frame_start,
                                                    unsigned num_bytes_in_frame, struct timeval frame_presentation_time,
                                                    unsigned num_remaining_bytes) {
    unsigned full_frame_size = fragmentation_offset + num_bytes_in_frame + num_remaining_bytes;
    unsigned char headers[4];
    headers[0] = 0;
    headers[1] = 16; /* bits AU-headers-length*/
    headers[2] = full_frame_size >> 5;
    headers[3] = (full_frame_size & 0x1F) << 3;
    ...
    MultiFramedRTPSink::do_special_frame_handling(fragmentation_offset, frame_start, num_bytes_in_frame,
