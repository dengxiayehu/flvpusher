...
OutPacketBuffer::OutPacketBuffer(unsigned preferred_packet_size, unsigned max_packet_size, unsigned max_buffer_size)
        : m_preferred(preferred_packet_size), m_max(max_packet_size), m_overflow_data_size(0) {
...
#ifdef XDEBUG
        LOGW("OutPacketBuffer::enqueue() warning: %d > %d", num_bytes, total_bytes_available());
#endif
        num_bytes = total_bytes_available();
    }

    if (cur_ptr() != from)
        memmove(cur_ptr(), from, num_bytes);
    increment(num_bytes);
}
