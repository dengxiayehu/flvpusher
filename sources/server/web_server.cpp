...
WebServerImpl::WebServerImpl(Config* conf) : m_conf(conf), m_heartbeat_thrd(NULL), m_recycle_thrd(NULL) {
...
int WebServerImpl::start(int listen_port, int server_threads) {
    if (listen_port < 1024 || listen_port > 65535 || server_threads < 1) {
        LOGE("Failed to start webserver (listen_port=%d,server_threads=%d)", listen_port, server_threads);
        return -1;
    }

    for (int i = 0; i < server_threads; ++i) {
        ServeParam* sp = new ServeParam;
        sp->server = mg_create_server(this, (mg_handler_t)ev_handler);
        sp->user = this;
        sp->thread_id = NULL;
    ...
    if (!is_path_absolute(STR(document_root))) {
        document_root = sprintf_("%s%c%s", STR(dirname_(abs_program)), DIRSEP, STR(document_root));
    }
    ...
    for (int i = 0; options[i]; i += 2) {
        FOR_VECTOR_ITERATOR(ServeParam*, m_serve_param, it) {
            const char* msg = mg_set_option((*it)->server, options[i], options[i + 1]);
            if (msg) {
                LOGE("Server#%d Failed to set option [%s] to [%s]: %s", distance(it, m_serve_param.end()), options[i],
                     options[i + 1], msg);
            }
    ...
    if (chdir(STR(document_root)) < 0) {
        LOGE("chdir to document_root \"%s\" failed: %s", STR(document_root), ERRNOMSG);
        return -1;
    }
    ...
    m_heartbeat_thrd = CREATE_THREAD_ROUTINE(heartbeat_routine, NULL, false);

    m_recycle_thrd = CREATE_THREAD_ROUTINE(recycle_routine, NULL, false);
...
int WebServerImpl::ev_handler(struct mg_connection* conn, enum mg_event ev) {
    switch (ev) {
        ...
        case MG_AUTH:
            return serve_auth(conn);
        default:
            return MG_FALSE;
    }
}

const static char* html_index =
        "<html><head><title>Welcome to flvpusher!</title><style>body{width:35em;margin:0 "
        "auto;font-family:Tahoma,Verdana,Arial,sans-serif;}</style></head><body><h1>Welcome to flvpusher!</h1><p>If "
        "you see this page, the server is successfully installed and working.</p><p><em>Thank you for "
        "using.</em></p></body></html>";
int WebServerImpl::serve_request(struct mg_connection* conn) {
    if (interrupt_cb())
        return MG_FALSE;

    if (!strcmp(conn->request_method, "GET") && (!strcmp(conn->uri, "/") || is_index(conn->uri + 1))) {
    ...
    if (st_type == ST_NETURAL || TAG_KIND_OF(st_type, ST_NET)) {
        return MG_FALSE;
    } else {
...
bool WebServerImpl::is_index(const char* uri) {
    const static char* candinates = "index.html,index.htm,index.shtml,index.cgi,index.php";
...
int WebServerImpl::send_response(struct mg_connection* conn, const char* code_desc, const char* content_type,
                                 int content_length, bool close_connection, uint8_t* content) {
    char buff[4096];
    int n;
...
void* WebServerImpl::heartbeat_routine(void* arg) {
    ...
    while (!interrupt_cb() && !heartbeat_failed) {
        Curl::request* req = Curl::request::build(
                Curl::OPTIONS, STR(sprintf_("http://127.0.0.1:%s", get_option(options, "listening_port"))), NULL, NULL,
                3, NULL, false);
        if (!req) {
        ...
        if (curl->perform(req, NULL) < 0 || req->response_code != 200) {
            LOGE("curl heartbeat failed (response_code=%d)", req->response_code);
            heartbeat_failed = true;
        }
        Curl::request::recycle(&req);

        if (!heartbeat_failed) {
            int curl_heartbeat_interval = DEFAULT_CURL_HEARTBEAT_INTERVAL;
            if (m_conf) {
    ...
    if (!interrupt_cb() && heartbeat_failed) {
        ...
        if (raise(SIGINT) < 0) {
            LOGE("raise SIGINT to stop self failed: %s", ERRNOMSG);
            set_interrupt(true);
            sleep_(1000);
            exit(EXIT_FAILURE);
        }
...
int WebServerImpl::recycle_func(void* opaque, const char* path) {
    ...
    if (is_dir(path)) {
        ...
        if (is_file(sprintf_("%s%chls_info.txt", STR(dirname_(path)), DIRSEP))) {
            if (end_with(path, ".ts")) {
                LOGD("Unlink \"%s\"", path);
...
int WebServer::send_response(struct mg_connection* conn, const char* code_desc, const char* content_type,
                             int content_length, bool close_connection, uint8_t* content) {
    return WebServerImpl::send_response(conn, code_desc, content_type, content_length, close_connection, content);
}
