...
Box::Box(uint32_t _sz, uint32_t _typ, uint64_t _largesz) : sz(_sz), typ(_typ), largesz(_largesz) {
    next = NULL;
    sub = NULL;
}
...
int Box::parse_box(Box*& pb, File* f, off_t curend) {
    while (!(f->eof() || f->cursor() >= curend)) {
        ...
        if (sz == 1) {
            ...
            LOGW("Box |%s| with too large size(%u), adjust it to %u", typ_str(typ), sz, sz_adjust);
            sz = sz_adjust;
        }
...
#ifdef XDEBUG
            LOGW("Unsupported box type: %s, size=%u (skipped)", typ_str(typ), sz);
#endif
...
FullBox::FullBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs) : Box(_sz, _typ), ver(_ver) {
...
FileTypeBox::FileTypeBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
MediaDataBox::MediaDataBox(uint32_t _sz, uint32_t _typ, uint64_t _largesz) : Box(_sz, _typ, _largesz) {
}
...
MovieBox::MovieBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
MovieHeaderBox::MovieHeaderBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
...
TrackBox::TrackBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
TrackHeaderBox::TrackHeaderBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
MediaBox::MediaBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
MediaHeaderBox::MediaHeaderBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
HandlerBox::HandlerBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
MediaInformationBox::MediaInformationBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
VideoMediaHeaderBox::VideoMediaHeaderBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
SoundMediaHeaderBox::SoundMediaHeaderBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
DataInformationBox::DataInformationBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
DataReferenceBox::DataReferenceBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
DataEntryUrlBox::DataEntryUrlBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
SampleTableBox::SampleTableBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
SampleDescriptionBox::SampleDescriptionBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
}
...
SampleEntry::SampleEntry(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
int SampleEntry::init(File* f) {
    ...
    for (int i = 0; i < 6; ++i) {
        if (!f->readui8(&reserved[i])) {
            LOGE("Read \"%s\" reserved[%d] failed", typ_str(typ), i);
            return -1;
        }
    }

    if (!f->readui16(&data_reference_index)) {
        LOGE("Read \"%s\" data_reference_index failed", typ_str(typ));
        return -1;
    }
...
VisualSampleEntry::VisualSampleEntry(uint32_t _sz, uint32_t _typ) : SampleEntry(_sz, _typ) {
}
...
int VisualSampleEntry::init(File* f) {
     ...
     */
    if (!f->readui16(&pre_defined, true)) {
        LOGE("Read \"%s\" pre_defined failed", typ_str(typ));
        return -1;
    }

    if (!f->readui16(&reserved1, true)) {
        LOGE("Read \"%s\" reserved1 failed", typ_str(typ));
        return -1;
    }

    for (int i = 0; i < 3; ++i) {
        if (!f->readui32(&pre_defined1[i], true)) {
            LOGE("Read \"%s\" pre_defined1[%d] failed", typ_str(typ), i);
            return -1;
        }
    }

    if (!f->readui16(&width, true)) {
        LOGE("Read \"%s\" width failed", typ_str(typ));
        return -1;
    }

    if (!f->readui16(&height, true)) {
        LOGE("Read \"%s\" height failed", typ_str(typ));
        return -1;
    }

    if (!f->readui32(&horizresolution, true)) {
        LOGE("Read \"%s\" horizresolution failed", typ_str(typ));
        return -1;
    }

    if (!f->readui32(&vertresolution, true)) {
        LOGE("Read \"%s\" vertresolution failed", typ_str(typ));
        return -1;
    }

    if (!f->readui32(&reserved2, true)) {
        LOGE("Read \"%s\" reserved2 failed", typ_str(typ));
        return -1;
    }

    if (!f->readui16(&frame_count, true)) {
        LOGE("Read \"%s\" frame_count failed", typ_str(typ));
        return -1;
    }

    if (!f->read_buffer((uint8_t*)compressorname, 32)) {
        LOGE("Read \"%s\" compressorname failed", typ_str(typ));
        return -1;
    }

    if (!f->readui16(&depth, true)) {
        LOGE("Read \"%s\" depth failed", typ_str(typ));
        return -1;
    }

    if (!f->readi16(&pre_defined2, true)) {
        LOGE("Read \"%s\" pre_defined2 failed", typ_str(typ));
        return -1;
    }
...
avcCBox::avcCBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
TimeToSampleBox::TimeToSampleBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    sample_count = NULL;
    sample_delta = NULL;
}
...
CompositionOffsetBox::CompositionOffsetBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs), entry_count(0), sample_count(NULL), sample_offset(NULL) {
}
...
SyncSampleBox::SyncSampleBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    sample_number = NULL;
}
...
SampleToChunkBox::SampleToChunkBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    first_chunk = NULL;
...
SampleSizeBox::SampleSizeBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    entry_size = NULL;
}
...
ChunkOffsetBox::ChunkOffsetBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    chunk_offset = NULL;
}
...
ChunkLargeOffsetBox::ChunkLargeOffsetBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs) {
    chunk_offset = NULL;
}
...
mp4aBox::mp4aBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
esdsBox::esdsBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs), to_confirm(false), audio_object_type(0), samplerate_idx(0), channel(0) {
}
...
int esdsBox::init(File* f) {
    ...
    generate_asc(asc, audio_object_type, samplerate_idx, channel);
#ifdef XDEBUG
    print_asc(asc);
#endif
...
EditBox::EditBox(uint32_t _sz, uint32_t _typ) : Box(_sz, _typ) {
}
...
EditListBox::EditListBox(uint32_t _sz, uint32_t _typ, uint8_t _ver, const uint24_t& _flgs)
        : FullBox(_sz, _typ, _ver, _flgs), elst_entry(NULL) {
}
