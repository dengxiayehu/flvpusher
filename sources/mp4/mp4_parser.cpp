...
MP4Parser::MP4Parser() : m_box(NULL), m_parsed_track(0), m_mp4(NULL) {
}
...
int MP4Parser::init() {
    ...
    if (m_parsed_track < NB_TRACK ||                             // Only one track in file, not support
        ...
        LOGE("Invalid audio(%u:%p) track or video(%u:%p) track", m_track[AUDIO].track_ID, m_track[AUDIO].mp4a,
    ...
    if (trak->elst && trak->elst->entry_count == 1) {
        ...
        int64_t media_time = trak->elst->ver == 1 ? trak->elst->elst_entry[0].media_time.i64
    ...
    uint8_t samplerate_idx = str_to_samplerate_idx(STR(sprintf_("%d", trak->mp4a->samplerate >> 16)));
    if (trak->esds->to_confirm) {
        // Confirm the asc which is generated by hand
        generate_asc(trak->esds->asc, 0x02 /*LC fixed*/, samplerate_idx, trak->mp4a->channelcount);

        trak->esds->to_confirm = false;
    } else {
        if (samplerate_idx != trak->esds->samplerate_idx || trak->mp4a->channelcount != trak->esds->channel) {
#ifdef XDEBUG
            LOGD("|esds|:idx=%u, channel=%u != |mp4a|:idx=%u, channel=%u", trak->esds->samplerate_idx,
                 trak->esds->channel, samplerate_idx, trak->mp4a->channelcount);
#endif
#if 0
      // |mp4a| wins
...
AVRational MP4Parser::get_vtime_base() const {
    const Track* trak = &m_track[VIDEO];
    return (AVRational){(int)(trak->duration / trak->timescale), (int)trak->stsz->sample_count};
}
...
int MP4Parser::init_tracks_from_box(Box* box, Track*& trak) {
    ...
    while (p != NULL) {
        if (p->typ == MKTAG4('t', 'k', 'h', 'd')) {
            ...
            trak->duration = mdhd->ver == 1 ? mdhd->duration.u64 : mdhd->duration.u32;
...
uint32_t MP4Parser::chunk_containing_sample(uint32_t sample_idx, const SampleToChunkBox* stsc,
                                            uint32_t& first_sample_in_chunk, ReadStatus::LocateChunkCache* lcc) {
    bool is_cont = lcc && lcc->cached_sample_idx + 1 == sample_idx ? true : false;

    // Cover only 1 entry in "stsc" box
    if (is_cont && !lcc->cached_entry_idx)
        is_cont = false;

    uint32_t prev_chunk = stsc->first_chunk[is_cont ? lcc->cached_entry_idx - 1 : 0];
    uint32_t prev_chunk_samples = stsc->sample_per_chunk[is_cont ? lcc->cached_entry_idx - 1 : 0];
    uint32_t total_samples = is_cont ? lcc->cached_total_samples : 0;
    uint32_t chunk;
    uint32_t range_samples;
    for (uint32_t i = is_cont ? lcc->cached_entry_idx : 1; i < stsc->entry_count; ++i) {
    ...
    chunk = prev_chunk + (sample_idx - total_samples) / prev_chunk_samples;

    // Get first sample in this chunk
    first_sample_in_chunk = total_samples + (chunk - prev_chunk) * prev_chunk_samples;
...
int MP4Parser::locate_sample(Track* trak, ReadStatus* rstatus, SampleEntry* sentry) {
    if (rstatus->sample_idx == trak->stsz->sample_count) {
        // No more frames to read
        return -1;
    }

    if (rstatus->stts.offset == trak->stts->sample_count[rstatus->stts.cnt]) {
        // Handle next sample_count in "stts"
        ++rstatus->stts.cnt;
        rstatus->stts.offset = 0;
    }

    if (trak->ctts && rstatus->ctts.offset == trak->ctts->sample_count[rstatus->ctts.cnt]) {
    ...
    uint32_t chunk = chunk_containing_sample(rstatus->sample_idx, trak->stsc, first_sample_in_chunk, &rstatus->lcc);
#ifdef XDEBUG
    LOGI("sample_idx: %u, chunk: %u, first_sample_in_chunk: %u", rstatus->sample_idx, chunk, first_sample_in_chunk);
#endif
    ...
    if (first_sample_in_chunk != rstatus->sample_idx) {
        for (uint32_t i = first_sample_in_chunk; i < rstatus->sample_idx; ++i) {
            rstatus->sample_offset += trak->stsz->entry_size[i];
        }
    }
    // Get sample's size
    uint32_t sample_sz = trak->stsz->entry_size[rstatus->sample_idx];
#ifdef XDEBUG
    LOGI("sample_idx: %u, FILE offset: 0x%08x, size: 0x%08x", rstatus->sample_idx, rstatus->sample_offset, sample_sz);
#endif
    ...
    frac_add(&rstatus->dts, trak->stts->sample_delta[rstatus->stts.cnt] * 1000);
    if (trak->ctts && sentry) {
        sentry->composition_time = trak->ctts->sample_offset[rstatus->ctts.cnt] * 1000 / trak->timescale;
    }
...
int MP4Parser::read_frame(File& file, Track* trak, SampleEntry* sentry, Frame* f) {
    if (!sentry || !f)
        return -1;
    ...
    if (!file.read_buffer(trak->video_track ? dat : dat + 7, sample_sz)) {
        LOGE("Read frame failed, sample_offset: %ld, sample_idx: %u, sample_sz: 0x%08x", sample_offset, sample_idx,
             sample_sz);
        SAFE_FREE(dat);
        return -1;
    }
    ...
    if (trak->video_track) {       // Video track
        ...
        while (nalu_offset < sample_sz) {
            ...
            if (nalu_typ == 0x07) {
                ...
                got_sps = true;
                is_keyframe = true;
            } else if (!got_sps && (nalu_typ == 0x08 || nalu_typ == 0x06 || nalu_typ == 0x05)) {
        ...
        if (is_keyframe && !got_sps) {
            ...
            uint8_t* complete_frame = (uint8_t*)malloc(4 + avc_dcr.sps_length + 4 + avc_dcr.pps_length + sample_sz);
            if (!complete_frame) {
            ...
            memcpy(complete_frame + 4, avc_dcr.sps, avc_dcr.sps_length);
            // Copy startcode and pps
            put_be32(complete_frame + 4 + avc_dcr.sps_length, 0x00000001);
            memcpy(complete_frame + 4 + avc_dcr.sps_length + 4, avc_dcr.pps, avc_dcr.pps_length);
            // Copy the rest of video frame
            memcpy(complete_frame + 4 + avc_dcr.sps_length + 4 + avc_dcr.pps_length, dat, sample_sz);
            SAFE_FREE(dat);
...
int MP4Parser::init_ffmpeg_context() {
    ...
    for (unsigned i = 0; i < NB_TRACK; ++i) {
        ...
        st->codec->codec_type = trak->video_track ? MEDIA_TYPE_VIDEO : MEDIA_TYPE_AUDIO;
        st->codec->codec_id = trak->video_track ? CODEC_ID_H264 : CODEC_ID_AAC;
...
int MP4Parser::mp4_read_packet(FormatContext* s, Packet* pkt) {
    ...
    SampleEntry sentry;
    Frame frame;
    if (locate_sample(trak, rstatus, &sentry) < 0 || read_frame(obj->m_file, trak, &sentry, &frame) < 0)
        return -1;
...
#ifdef XDEBUG
    LOGD("%s pkt->pts=%lld, pkt->dts=%lld (composition_time=%u), pkt->size=%d, pkt->duration: %d, current_sample#=%d, "
         "total samples#=%d",
         pkt->stream_index == AUDIO ? "AUDIO" : "VIDEO", pkt->pts, pkt->dts, frame.get_composition_time(), pkt->size,
         pkt->duration, rstatus->sample_idx, trak->stsz->sample_count);
#endif
    return 0;
}

void MP4Parser::print_ReadStatus(const ReadStatus& rs) {
    LOGD("%u %u %u %u {%d, %d} %u %u {%u, %u, %u} %lld", rs.stts.cnt, rs.stts.offset, rs.ctts.cnt, rs.ctts.offset,
         rs.dts.num, rs.dts.den, rs.shift_time, rs.sample_idx, rs.lcc.cached_sample_idx, rs.lcc.cached_entry_idx,
         rs.lcc.cached_total_samples, rs.sample_offset);
}
