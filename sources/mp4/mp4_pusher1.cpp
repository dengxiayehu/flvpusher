...
MP4Pusher1::MP4Pusher1(const std::string& input, MediaSink*& sink)
        : MediaPusher(input, sink), m_parser(NULL), m_width(0), m_height(0) {
}
...
int MP4Pusher1::prepare() {
    ...
    if (m_sink->type() == MediaSink::RTMP_SINK && !send_metadata()) {
...
int MP4Pusher1::parsed_frame_cb(void* opaque, Frame* f, int is_video) {
    ...
    obj->on_frame(dts, f->get_data(), f->get_data_length(), is_video, f->get_composition_time());

    if (is_video) {
        if (obj->m_sink->send_video(dts, f->get_data(), f->get_data_length(), f->get_composition_time()) < 0) {
        ...
        if (obj->m_sink->send_audio(dts, f->get_data(), f->get_data_length()) < 0) {
            LOGE("Send audio data to %sserver failed", STR(obj->m_sink->type_str()));
            ret = -1;
        }
    }
...
int MP4Pusher1::send_metadata() {
    ...
    put_amf_obj_end(p);
    return ((RtmpSink*)m_sink)
            ->send_rtmp_pkt(RTMP_PACKET_TYPE_INFO, 0 /* metadata's timestamp is always 0*/, buff, p - buff);
}
